...
LocalStorageManagerAsync::localStorageManager() const
{
    Q_D(const LocalStorageManagerAsync);
...
void LocalStorageManagerAsync::init()
{
    ...
    d->m_pLocalStorageManager = new LocalStorageManager(
        d->m_account,
        d->m_startupOptions);
...
void LocalStorageManagerAsync::onUpdateUserRequest(User user, QUuid requestId)
{
    ...
    try
    {
        ...
        bool res = d->m_pLocalStorageManager->updateUser(
            user,
            errorDescription);
...
void LocalStorageManagerAsync::onExpungeUserRequest(User user, QUuid requestId)
{
    ...
    try
    {
        ...
        bool res = d->m_pLocalStorageManager->expungeUser(
            user,
            errorDescription);
    ...
    Notebook notebook, QUuid requestId)
{
    ...
    try
    {
        ...
        bool res = d->m_pLocalStorageManager->addNotebook(
            notebook,
            errorDescription);
    ...
    LocalStorageManager::NoteCountOptions options, QUuid requestId)
{
    ...
    {
        ErrorString errorDescription;
        int count = d->m_pLocalStorageManager->noteCount(
            errorDescription,
            options);
    ...
    Tag tag, LocalStorageManager::NoteCountOptions options, QUuid requestId)
{
    ...
    try
    {
        ...
        {
            Q_EMIT getNoteCountPerTagComplete(
                count,
                tag,
                options,
                requestId);
    ...
    QUuid requestId)
{
    ...
    try
    {
        ...
        if (shouldCheckForTagListUpdate)
        {
            ...
            if (!tagListUpdated)
            {
                for(const auto & prevTagLocalUid: qAsConst(previousTagLocalUids))
                {
    ...
    Note note, LocalStorageManager::GetNoteOptions options, QUuid requestId)
{
    ...
    try
    {
        ...
        if (d->m_useCache)
        {
            ...
            const Note * pNote = d->m_pLocalStorageCacheManager->findNote(
                uid,
                wu);
...
void LocalStorageManagerAsync::onExpungeNoteRequest(Note note, QUuid requestId)
{
    ...
    try
    {
        ...
        bool res = d->m_pLocalStorageManager->expungeNote(
            note,
            errorDescription);
    ...
    try
    {
        ...
        if (d->m_useCache)
        {
            ...
            if (tagHasGuid || !tag.localUid().isEmpty())
            {
                ...
                const Tag * pTag = d->m_pLocalStorageCacheManager->findTag(
                    uid,
                    wg);
        ...
        if (!foundTagInCache)
        {
            bool res = d->m_pLocalStorageManager->findTag(
                tag,
                errorDescription);
...
void LocalStorageManagerAsync::onGetSavedSearchCountRequest(QUuid requestId)
{
    ...
    {
        ErrorString errorDescription;
        int count = d->m_pLocalStorageManager->savedSearchCount(
            errorDescription);
        if (count < 0) {
    ...
    SavedSearch search, QUuid requestId)
{
    ...
    try
    {
        ...
        bool res = d->m_pLocalStorageManager->addSavedSearch(
            search,
            errorDescription);
