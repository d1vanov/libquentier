    ...
    const User & user, ErrorString & errorDescription)
{
    ErrorString errorPrefix(
        QT_TR_NOOP("Can't insert user data into the local storage database"));
...
{
    ErrorString errorPrefix(
        QT_TR_NOOP("Can't update user data in the local storage database"));
    ...
    User & user, ErrorString & errorDescription) const
{
    ...
    ErrorString errorPrefix(
        QT_TR_NOOP("Can't find user data in the local storage database"));
...
{
    ErrorString errorPrefix(
        QT_TR_NOOP("Can't mark user data as deleted in the local storage "
                   "database"));
...
{
    ErrorString errorPrefix(
        QT_TR_NOOP("Can't expunge user data from the local storage database"));
    ...
    const Account & account, const LocalStorageManager::StartupOptions options)
{
    ...
    {
        ErrorString error(
            QT_TR_NOOP("Can't initialize tables in the local storage database"));
...
{
    ErrorString errorPrefix(
        QT_TR_NOOP("Can't insert notebook data into the local storage "
                   "database"));
...
{
    ErrorString errorPrefix(
        QT_TR_NOOP("Can't update notebook data in the local storage database"));
    ...
    const QString & linkedNotebookGuid) const
{
    ...
    return listObjects<Notebook, LocalStorageManager::ListNotebooksOrder>(
        flag,
        errorDescription,
        limit,
        offset,
    ...
    const LocalStorageManager::OrderDirection & orderDirection) const
{
    ...
    return listObjects<LinkedNotebook, LocalStorageManager::ListLinkedNotebooksOrder>(
        flag,
        errorDescription,
        limit,
        offset,
...
{
    ErrorString errorPrefix(
        QT_TR_NOOP("Can't get the number of notes per tag in the local storage "
                   "database"));
...
{
    ErrorString errorPrefix(
        QT_TR_NOOP("Can't get note counts for all tags in the local storage "
                   "database"));
    ...
    while(query.next())
    {
        ...
        if (Q_UNLIKELY(tagLocalUidIndex < 0))
        {
            errorDescription.base() = errorPrefix.base();
            errorDescription.appendBase(
                QT_TR_NOOP("can't find the local uid of tag in the result of "
                           "SQL query"));
            QNWARNING(errorDescription);
            return false;
        }
        ...
        if (Q_UNLIKELY(tagLocalUid.isEmpty()))
        {
            errorDescription.base() = errorPrefix.base();
            errorDescription.appendBase(
                QT_TR_NOOP("the local uid of tag from the result of SQL query "
                           "is empty"));
            QNWARNING(errorDescription);
            return false;
        }
        ...
        if (Q_UNLIKELY(noteCountIndex < 0))
        {
            errorDescription.base() = errorPrefix.base();
            errorDescription.appendBase(
                QT_TR_NOOP("can't find the note count for tag in the result of "
                           "SQL query"));
            QNWARNING(errorDescription);
            return false;
        }
        ...
        if (Q_UNLIKELY(noteCount < 0))
        {
            errorDescription.base() = errorPrefix.base();
            errorDescription.appendBase(
                QT_TR_NOOP("the note count for tag from the result of SQL "
                           "query is negative"));
            QNWARNING(errorDescription);
            return false;
        }
...
{
    ErrorString errorPrefix(
        QT_TR_NOOP("Can't get the number of notes per notebooks and tags in "
                   "the local storage database"));
    ...
    if (noteHasGuid)
    {
        ...
        if (localUid.isEmpty())
        {
            ...
            if (res || !localUid.isEmpty())
            {
                errorDescription.base() = errorPrefix.base();
                errorDescription.appendBase(
                    QT_TR_NOOP("found already existing note with the same guid"));
    ...
    const LocalStorageManager::OrderDirection & orderDirection) const
{
    ...
    ErrorString error;
    QList<Note> notes = listObjects<Note, LocalStorageManager::ListNotesOrder>(
    ...
    const LocalStorageManager::OrderDirection & orderDirection) const
{
    ...
    ErrorString error;
    tags = listObjects<Tag, LocalStorageManager::ListTagsOrder>(
    ...
    const QString & linkedNotebookGuid) const
{
    ...
    return listObjects<Tag, LocalStorageManager::ListTagsOrder>(
        flag,
        errorDescription,
        limit,
        offset,
    ...
    ErrorString & errorDescription) const
{
    ...
    if (note.hasNotebookLocalUid()) {
        ...
        QNTRACE("Notebook local uid taken from note: " << notebookLocalUid);
        return true;
    }
...
bool LocalStorageManagerPrivate::complementTagParentInfo(
    Tag & tag, ErrorString & errorDescription)
{
    QNDEBUG("LocalStorageManagerPrivate::complementTagParentInfo: " << tag);
    ...
    QNTRACE("Tag's parent " << otherColumn << " was retrieved: " << otherUid);
    ...
    ErrorString & errorDescription) const
{
    ...
    if (!resourceDataFile.exists())
    {
        QNDEBUG("Resource data file doesn't exist: "
...
#undef CHECK_AND_SET_NOTE_PROPERTY
    ...
    {
        QNTRACE("Found thumbnail data for note within the SQL record");
    ...
    ErrorString & errorDescription) const
{
    ...
    QNTRACE("Found " << numResources << " resources");
    ...
    const bool updateResourceBinaryData, ErrorString & errorDescription)
{
    ...
    QNDEBUG("Starting to process the query results");

    QString resourceLocalUidProperty = QStringLiteral("resourceLocalUid");
    while(query.next())
    {
        QNDEBUG("Processing the query result");
        QSqlRecord record = query.record();
...
{
    QNDEBUG("LocalStorageManagerPrivate::clearCachedQueries");
...
template <>
QString LocalStorageManagerPrivate::orderByToSqlTableColumn<LocalStorageManager::ListNotesOrder>(
    const LocalStorageManager::ListNotesOrder & order) const
{
    QString result;
...
template <>
QString LocalStorageManagerPrivate::orderByToSqlTableColumn<LocalStorageManager::ListNotebooksOrder>(
    const LocalStorageManager::ListNotebooksOrder & order) const
{
    QString result;
...
template <>
QString LocalStorageManagerPrivate::orderByToSqlTableColumn<LocalStorageManager::ListLinkedNotebooksOrder>(
    const LocalStorageManager::ListLinkedNotebooksOrder & order) const
{
    QString result;
...
template <>
QString LocalStorageManagerPrivate::orderByToSqlTableColumn<LocalStorageManager::ListTagsOrder>(
    const LocalStorageManager::ListTagsOrder & order) const
{
    QString result;
...
template <>
QString LocalStorageManagerPrivate::orderByToSqlTableColumn<LocalStorageManager::ListSavedSearchesOrder>(
    const LocalStorageManager::ListSavedSearchesOrder & order) const
{
    QString result;
