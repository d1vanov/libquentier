    ...
    const qevercloud::User & user, ErrorString & errorDescription)
{
    ...
    query.bindValue(
        QStringLiteral(":userDeletionTimestamp"), *user.deleted());

    query.bindValue(QStringLiteral(":userIsLocal"), (user.isLocalOnly() ? 1 : 0));
    ...
    const qevercloud::User & user, ErrorString & errorDescription)
{
    ...
    const QString queryString = QStringLiteral("DELETE FROM Users WHERE id=:id");
    QSqlQuery query(m_sqlDatabase);
    bool res = query.prepare(queryString);
    DATABASE_CHECK_AND_SET_ERROR()
    ...
    ErrorString & errorDescription)
{
    ...
    const qint32 highestSupportedVersion = highestSupportedLocalStorageVersion();
    ...
    qevercloud::Note & note, ErrorString & errorDescription)
{
    ...
    note.setParentLocalId(notebookLocalId);
    ...
    ErrorString & errorDescription)
{
    ...
    if (note.guid()) {
        ...
        if (localId.isEmpty()) {
            error.clear();
            if (!getNoteLocalIdForGuid(id, localId, error) ||
                localId.isEmpty())
            {
                errorDescription.base() = errorPrefix.base();
                errorDescription.appendBase(error.base());
    ...
    ErrorString & errorDescription) const
{
    ...
    queryString += QString::fromUtf8("WHERE %1 = '%2'").arg(column, id);
    QSqlQuery query(m_sqlDatabase);
    bool res = query.exec(queryString);
    DATABASE_CHECK_AND_SET_ERROR()
    ...
    QHash<QString, int> resourceIndexPerLocalId;
    ...
    while (query.next()) {
        ...
        if (withResourceMetadata) {
            const int resourceLocalIdIndex =
                rec.indexOf(QStringLiteral("resourceLocalUid"));

            if (resourceLocalIdIndex >= 0) {
                const QVariant value = rec.value(resourceLocalIdIndex);
                if (!value.isNull()) {
                    const QString resourceLocalId = value.toString();

                    const auto it =
                        resourceIndexPerLocalId.find(resourceLocalId);

                    const bool resourceIndexNotFound =
                        (it == resourceIndexPerLocalId.end());

                    if (resourceIndexNotFound) {
                        const int resourceIndexInList = resources.size();
                        resourceIndexPerLocalId[resourceLocalId] =
                            resourceIndexInList;
                    ...
                    resource.setParentLocalId(note.localId());
...
QList<qevercloud::Note> LocalStorageManagerPrivate::listNotesPerNotebooksAndTags(
    const QStringList & notebookLocalIds, const QStringList & tagLocalIds,
    ...
    qevercloud::Tag & tag, ErrorString & errorDescription)
{
    ...
    if (tag.guid()) {
        ...
        if (localId.isEmpty()) {
            error.clear();
            if (getTagLocalIdForGuid(id, localId, error) &&
                !localId.isEmpty())
            {
                errorDescription.base() = errorPrefix.base();
                errorDescription.appendBase(
    ...
    ErrorString & errorDescription) const
{
    ...
    if (!query.next()) {
        QNDEBUG(
            "local_storage", "Found no resources in the local storage");
        return 0;
    }
    ...
    ErrorString & errorDescription) const
{
    ...
    size_t counter = 0;
    while (query.next()) {
        fillResourceFromSqlRecord(query.record(), foundResource);
        ++counter;
    }
    ...
    qevercloud::Resource & resource, ErrorString & errorDescription)
{
    ...
    resource.setParentLocalId(noteLocalId);
    ...
    ErrorString & errorDescription) const
{
    ...
    if (!query.next()) {
        QNDEBUG(
            "local_storage",
            "Found no saved searches in the local storage");
        return 0;
    }
    ...
    qevercloud::SavedSearch & search, ErrorString & errorDescription)
{
    ...
    const ErrorString errorPrefix(QT_TR_NOOP(
        "Can't expunge saved search from the local storage"));
    ...
    qevercloud::Resource & resource, ErrorString & errorDescription)
{
    ...
    if (!resource.noteGuid() && resource.parentLocalId().isEmpty()) {
        errorDescription.base() = errorPrefix.base();
        errorDescription.appendBase(
    ...
    QString noteLocalId = resource.parentLocalId();
...
#define BIND_RESTRICTION(name)                                                 \
    query.bindValue(                                                           \
        QStringLiteral(":" #name),                                             \
        (notebookRestrictions.name()                                           \
             ? (*notebookRestrictions.name() ? 1 : 0)                          \
             : nullValue))
    ...
    ErrorString & errorDescription)
{
    ...
    query.bindValue(
        ...
        (sharedNotebook.serviceCreated()
             ? *sharedNotebook.serviceCreated()
             : nullValue));

    query.bindValue(
        QStringLiteral(":sharedNotebookModificationTimestamp"),
        (sharedNotebook.serviceUpdated()
             ? *sharedNotebook.serviceUpdated()
             : nullValue));
    ...
    query.bindValue(
        ...
        (sharedNotebook.recipientSettings()
         ? (sharedNotebook.recipientSettings()->reminderNotifyEmail()
            ? (*sharedNotebook.recipientSettings()->reminderNotifyEmail() ? 1 : 0)
            : nullValue)
         : nullValue));
    ...
    query.bindValue(
        ...
        (sharedNotebook.recipientSettings()
         ? (sharedNotebook.recipientSettings()->reminderNotifyInApp()
            ? (*sharedNotebook.recipientSettings()->reminderNotifyInApp() ? 1 : 0)
            : nullValue)
         : nullValue));
    ...
    query.bindValue(
        ...
        (sharedNotebook.serviceAssigned()
             ? *sharedNotebook.serviceAssigned()
             : nullValue));
    ...
    const qevercloud::Notebook & notebook, ErrorString & errorDescription)
{
    ...
    // Insert or replace common Notebook data
    {
        ...
        query.bindValue(
            ...
            (notebook.published()
             ? static_cast<int>(*notebook.published())
             : nullValue));
        ...
        query.bindValue(
            ...
            (notebook.businessNotebook()
             ? (notebook.businessNotebook()->privilege()
                ? static_cast<int>(*notebook.businessNotebook()->privilege())
                : nullValue)
             : nullValue));
        ...
        query.bindValue(
            ...
            (notebook.businessNotebook()
             ? (notebook.businessNotebook()->recommended()
                ? static_cast<int>(*notebook.businessNotebook()->recommended())
                : nullValue)
             : nullValue));
        ...
        query.bindValue(
            ...
            (notebook.recipientSettings()
             ? (notebook.recipientSettings()->inMyList()
                ? static_cast<int>(*notebook.recipientSettings()->inMyList())
                : nullValue)
             : nullValue));
...
bool LocalStorageManagerPrivate::checkAndPrepareNotebookCountQuery() const
{
    ...
    QNDEBUG(
        "local_storage",
        "Preparing SQL query to get the count of notebooks");

    m_getNotebookCountQuery = QSqlQuery(m_sqlDatabase);
    const QString queryString = QStringLiteral("SELECT COUNT(*) FROM Notebooks");
    ...
    ErrorString & errorDescription) const
{
    ...
    if (!resource.parentLocalId().isEmpty()) {
        noteLocalId = resource.parentLocalId();
        return true;
    }
    ...
    ErrorString & errorDescription) const
{
    ...
    if (!note.parentLocalId().isEmpty()) {
        notebookLocalId = note.parentLocalId();
        QNTRACE(
            "local_storage",
    ...
    ErrorString & errorDescription) const
{
    ...
    if (note.parentLocalId().isEmpty()) {
        errorDescription.base() = errorPrefix.base();
        errorDescription.appendBase(QT_TR_NOOP(
            "note has neither parent local id nor notebook guid"));
        QNDEBUG("local_storage", errorDescription << ", note: " << note);
        return false;
    }

    const QString notebookLocalId = sqlEscapeString(note.parentLocalId());
    ...
    ErrorString & errorDescription)
{
    ...
    QString notebookLocalUid =
        (!note.parentLocalId().isEmpty()
         ? sqlEscapeString(note.parentLocalId())
         : QString());
    ...
    // Update common table with Note properties
    {
        ...
        query.bindValue(
            QStringLiteral(":updateSequenceNumber"),
            (note.updateSequenceNum() ? *note.updateSequenceNum()
                                      : nullValue));

        query.bindValue(
            QStringLiteral(":isDirty"),
            (note.isLocallyModified() ? 1 : 0));
        ...
        const QByteArray thumbnailData = [&note] {
            const auto it = note.localData().constFind(
                QStringLiteral("thumbnailData"));
    ...
    qevercloud::Tag & tag, ErrorString & errorDescription)
{
    ...
    if (tag.parentGuid() && !tag.parentLocalId().isEmpty()) {
        QNDEBUG(
            "local_storage",
    ...
    const QString uid =
        (tag.parentGuid() ? *tag.parentGuid() : tag.parentLocalId());

    const QString queryString = QString::fromUtf8("SELECT %1 FROM Tags WHERE %2='%3'")
    ...
    const bool setResourceBinaryData, const bool useSeparateTransaction)
{
    ...
    const QString noteLocalId = resource.parentLocalId();
    ...
    if (resource.attributes() && !insertOrReplaceResourceAttributes(
    ...
    const qevercloud::Resource & resource, ErrorString & errorDescription)
{
    ...
    if (resource.data() && resource.data()->body()) {
        ...
        if (!writeResourceBinaryDataToFile(
                resourceLocalId, resource.parentLocalId(),
                *resource.data()->body(),
    ...
    const QString alternateDataStoragePath = storagePath +
        QStringLiteral("/Resources/alternateData/") + resource.parentLocalId() +
    ...
    const QString dataStoragePath = storagePath +
        QStringLiteral("/Resources/data/") + resource.parentLocalId() +
        QStringLiteral("/") + resourceLocalId + QStringLiteral(".dat");
    ...
    ErrorString & errorDescription)
{
    ...
    query.bindValue(QStringLiteral(":noteLocalUid"), resource.parentLocalId());
    ...
    const auto indexInNote = resourceIndexInNote(resource);

    query.bindValue(
        QStringLiteral(":resourceIndexInNote"), (indexInNote ? *indexInNote : nullValue));
    ...
    const qevercloud::Resource & resource, ErrorString & errorDescription)
{
    ...
    query.bindValue(QStringLiteral(":localNote"), resource.parentLocalId());
    ...
    const qevercloud::Resource & resource, ErrorString & errorDescription)
{
    QNDEBUG(
        ...
        "LocalStorageManagerPrivate::removeResourceDataFiles: "
            ...
            << ", note local id = " << resource.parentLocalId());

    if (Q_UNLIKELY(resource.parentLocalId().isEmpty())) {
        errorDescription.setBase(
            QT_TR_NOOP("the resource has no note local id set"));
    ...
    checkAndPrepareDeleteResourceFromResourceAttributesApplicationDataFullMapQuery()
{
    ...
    const bool res =
        m_deleteResourceFromResourceAttributesApplicationDataFullMapQuery.prepare(
            queryString);
    ...
    const qevercloud::SavedSearch & search, ErrorString & errorDescription)
{
    ...
    query.bindValue(
        QStringLiteral(":guid"),
        (search.guid() ? *search.guid() : nullValue));

    query.bindValue(
        QStringLiteral(":name"),
        (search.name() ? *search.name() : nullValue));
    ...
    query.bindValue(
        QStringLiteral(":isDirty"),
        (search.isLocallyModified() ? 1 : 0));

    query.bindValue(
        QStringLiteral(":isLocal"),
        (search.isLocalOnly() ? 1 : 0));

    query.bindValue(
        QStringLiteral(":includeAccount"),
        (search.scope()
         ? (search.scope()->includeAccount()
    ...
    query.bindValue(
        ...
        (search.scope()
         ? (search.scope()->includePersonalLinkedNotebooks()
            ? (*search.scope()->includePersonalLinkedNotebooks() ? 1 : 0)
            : nullValue)
         : nullValue));
    ...
    query.bindValue(
        ...
        (search.scope()
         ? (search.scope()->includeBusinessLinkedNotebooks()
            ? (*search.scope()->includeBusinessLinkedNotebooks() ? 1 : 0)
            : nullValue)
         : nullValue));
    ...
    ErrorString & errorDescription) const
{
    ...
    while (query.next()) {
        ...
        if (localNoteIndex >= 0) {
            ...
            if (!noteLocalId.isEmpty()) {
                Q_UNUSED(noteLocalIdsByTagLocalId[tagLocalId].insert(
                    noteLocalId))
            }
        }
    }
    ...
    qevercloud::Resource & resource, ErrorString & errorDescription) const
{
    ...
    if (resource.data()) {
        ...
        auto status = readResourceBinaryDataFromFile(
            resource.localId(), resource.parentLocalId(),
    ...
    if (!resourceDataFile.exists()) {
        ...
        if (isAlternateDataBody) {
            ...
            if (prevAlternateDataFileInfo.exists() &&
                ...
                const QString prevAlternateDataFilePath = QDir::toNativeSeparators(
                    prevAlternateDataFileInfo.absoluteFilePath());
...
#define CHECK_AND_SET_RESOURCE_PROPERTY(property, type, localType, setter)     \
    ...
    CHECK_AND_SET_RESOURCE_PROPERTY(resourceIsDirty, int, bool, setLocallyModified);
    CHECK_AND_SET_RESOURCE_PROPERTY(noteGuid, QString, QString, setNoteGuid);

    CHECK_AND_SET_RESOURCE_PROPERTY(
        localNote, QString, QString, setParentLocalId);
...
void LocalStorageManagerPrivate::
    fillNoteAttributesApplicationDataKeysOnlyFromSqlRecord(
        const QSqlRecord & rec, qevercloud::NoteAttributes & attributes) const
{
    const int keysOnlyIndex = rec.indexOf(QStringLiteral("applicationDataKeysOnly"));
    if (keysOnlyIndex < 0) {
        return;
    }
    ...
    const QSqlRecord & rec, qevercloud::NoteAttributes & attributes) const
{
    ...
    QMap<QString, QString> & classifications = *attributes.mutableClassifications();
    QStringList keysList, valuesList;
...
#define FIND_AND_SET_USER_PROPERTY(                                            \
    ...
    FIND_AND_SET_USER_PROPERTY(
        userIsLocal, setLocalOnly, int, bool, isRequired)
...
#undef FIND_AND_SET_USER_PROPERTY
    ...
    if (user.attributes()) {
        ...
        attributes.setRecentMailedAddresses(userAttributes.recentMailedAddresses());
    }
...
#define FIND_AND_SET_ACCOUNT_LIMIT(property, setter, type, localType)          \
    ...
    FIND_AND_SET_ACCOUNT_LIMIT(
        noteSizeMax, setNoteSizeMax, qint64, qint64)
    ...
    qevercloud::Resource & resource, ErrorString & errorDescription) const
{
    ...
    if (!resource.noteGuid()) {
        const QString noteLocalId = sqlEscapeString(resource.parentLocalId());
    ...
    else if (resource.parentLocalId().isEmpty()) {
    ...
    const bool updateResourceBinaryData, ErrorString & errorDescription)
{
    ...
    const QString listNoteResourcesQueryString =
        QString::fromUtf8(
            "SELECT Resources.resourceLocalUid, resourceGuid, "
    ...
    QList<qevercloud::Resource> previousNoteResources;

    QNDEBUG("local_storage", "Starting to process the query results");

    const QString resourceLocalUidProperty = QStringLiteral("resourceLocalUid");
    while (query.next()) {
        QNDEBUG("local_storage", "Processing the query result");
        const QSqlRecord record = query.record();

        const int resourceLocalUidIndex =
            record.indexOf(resourceLocalUidProperty);

        if (resourceLocalUidIndex < 0) {
            errorDescription.base() = errorPrefix.base();
            errorDescription.appendBase(
                QT_TR_NOOP("can't retrieve the resource local id from "
                           "the query result"));
            QNWARNING(
                "local_storage",
                errorDescription
        ...
        resource.setParentLocalId(
            record.value(resourceLocalUidIndex).toString());
    ...
    QStringList localIdsForResourcesRemovedFromNote;
    ...
    for (int i = 0; i < numPreviousResources; ++i) {
        const auto & previousNoteResource = previousNoteResources[i];

        bool foundResource = false;
        for (int j = 0; j < numResources; ++j) {
            const auto & resource = updatedNoteResources[j];
            if (resource.localId() != previousNoteResource.localId()) {
                continue;
            }
            ...
            bool changed =
                (resource.parentLocalId() != previousNoteResource.parentLocalId());

#define COMPARE_RESOURCE_PROPERTY(property)                                    \
    changed = changed ||                                                       \
        ((resource.property() && previousNoteResource.property() &&            \
          (*resource.property() != *previousNoteResource.property())) ||       \
         (resource.property() != previousNoteResource.property()))

            COMPARE_RESOURCE_PROPERTY(guid);
            COMPARE_RESOURCE_PROPERTY(noteGuid);
            COMPARE_RESOURCE_PROPERTY(updateSequenceNum);
            COMPARE_RESOURCE_PROPERTY(mime);
            COMPARE_RESOURCE_PROPERTY(width);
            COMPARE_RESOURCE_PROPERTY(height);
            COMPARE_RESOURCE_PROPERTY(attributes);

#undef COMPARE_RESOURCE_PROPERTY

#define COMPARE_RESOURCE_DATA_PROPERTY(data, property)                         \
    changed = changed ||                                                       \
        ((resource.data() && previousNoteResource.data() &&                    \
          resource.data()->property() &&                                       \
          previousNoteResource.data()->property() &&                           \
          (*resource.data()->property() !=                                     \
           *previousNoteResource.data()->property())) ||                       \
         (resource.data() != previousNoteResource.data()) ||                   \
         (resource.data() && previousNoteResource.data() &&                    \
          (resource.data()->property() !=                                      \
           previousNoteResource.data()->property())))
            ...
            COMPARE_RESOURCE_DATA_PROPERTY(data, size);
            COMPARE_RESOURCE_DATA_PROPERTY(data, bodyHash);
            COMPARE_RESOURCE_DATA_PROPERTY(recognition, size);
            COMPARE_RESOURCE_DATA_PROPERTY(recognition, bodyHash);
            COMPARE_RESOURCE_DATA_PROPERTY(alternateData, size);
            COMPARE_RESOURCE_DATA_PROPERTY(alternateData, bodyHash);

#undef COMPARE_RESOURCE_DATA_PROPERTY

            changed |=
                (resource.isLocallyModified() !=
                 previousNoteResource.isLocallyModified());

            changed |=
                (resource.isLocalOnly() != previousNoteResource.isLocalOnly());

            changed |=
                (resourceIndexInNote(resource) !=
                 resourceIndexInNote(previousNoteResource));
        ...
        if (!foundResource) {
            localIdsForResourcesRemovedFromNote
    ...
    for (int j = 0; j < numResources; ++j) {
        ...
        bool foundResource = false;
        for (int i = 0; i < numPreviousResources; ++i) {
            const auto & previousResource = previousNoteResources[i];
            if (resource.localId() != previousResource.localId()) {
                continue;
            }
    ...
    if (!localIdsForResourcesRemovedFromNote.isEmpty()) {
        QString removeResourcesQueryString =
            QString::fromUtf8(
                ...
                .arg(localIdsForResourcesRemovedFromNote.join(
                    QStringLiteral(",")));
        res = query.exec(removeResourcesQueryString);
        DATABASE_CHECK_AND_SET_ERROR()

        for (const auto & localId:
             qAsConst(localIdsForResourcesRemovedFromNote)) {
            qevercloud::Resource resource;
            resource.setLocalId(localId);
            resource.setParentLocalId(noteLocalId);
    ...
    if (!updateResourceBinaryData && (numAddedResources != 0)) {
        ...
        QStringList addedResourcesLocalIds;
        addedResourcesLocalIds.reserve(numAddedResources);
        for (const auto & resource: qAsConst(addedResources)) {
            addedResourcesLocalIds << resource.localId();
        }
        ...
        QNWARNING(
            ...
            errorDescription
                << ", note local id = " << noteLocalId
                << ", new resources local ids: "
                << addedResourcesLocalIds.join(QStringLiteral(", "))
                << ", note resources: " << updatedNoteResources);
        return false;
    }
    ...
    for (int i = 0; i < numAddedResources; ++i) {
        const auto & resource = addedResources[i];
...
{
    return
        sharedNotebookIndexInNotebook(lhs) < sharedNotebookIndexInNotebook(rhs);
}
...
{
    return resourceIndexInNote(lhs) < resourceIndexInNote(rhs);
}
