...
{
    if (m_type == Selection) {
        errorDescription.setBase(QT_TRANSLATE_NOOP("", "Can't commit the transaction of selection type"));
        return false;
    }
    ...
    if (!res) {
        errorDescription.setBase(QT_TRANSLATE_NOOP("", "Can't commit the SQL transaction"));
...
{
    if (m_type != Selection) {
        errorDescription.setBase(QT_TRANSLATE_NOOP("", "Only transactions used for selection queries should be "
    ...
    bool res = query.exec(QStringLiteral("END"));
    if (!res) {
        errorDescription.setBase(QT_TRANSLATE_NOOP("", "Can't end the SQL transaction"));
        errorDescription.details() = query.lastError().text();
        QNWARNING(errorDescription << QStringLiteral(", full last query error: ") << query.lastError());
        return false;
    }
