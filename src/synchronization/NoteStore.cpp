                             ...
                             const QString & linkedNotebookAuthToken)
{
    ...
    {
        return processEdamUserExceptionForNote(note, userException,
                                               UserExceptionSource::Creation,
                                               errorDescription);
    }
    catch(const qevercloud::EDAMSystemException & systemException)
...
qint32 NoteStore::createTag(Tag & tag,
                            ErrorString & errorDescription,
                            qint32 & rateLimitSeconds,
                            const QString & linkedNotebookAuthToken)
{
    try
    {
        tag.qevercloudTag() = m_pQecNoteStore->createTag(tag.qevercloudTag(),
                                                         linkedNotebookAuthToken);
        return 0;
    }
    catch(const qevercloud::EDAMUserException & userException)
...
qint32 NoteStore::updateTag(Tag & tag,
                            ErrorString & errorDescription,
                            qint32 & rateLimitSeconds,
                            const QString & linkedNotebookAuthToken)
{
    try
    {
        qint32 usn = m_pQecNoteStore->updateTag(tag.qevercloudTag(),
                                                linkedNotebookAuthToken);
...
qint32 NoteStore::createSavedSearch(SavedSearch & savedSearch,
                                    ErrorString & errorDescription,
                                    qint32 & rateLimitSeconds)
{
    try
    {
        savedSearch.qevercloudSavedSearch() =
            m_pQecNoteStore->createSearch(savedSearch.qevercloudSavedSearch());
        return 0;
    }
    catch(const qevercloud::EDAMUserException & userException)
    {
...
qint32 NoteStore::updateSavedSearch(SavedSearch & savedSearch,
                                    ErrorString & errorDescription,
                                    qint32 & rateLimitSeconds)
{
    try
...
qint32 NoteStore::getSyncState(qevercloud::SyncState & syncState,
                               ErrorString & errorDescription,
                               qint32 & rateLimitSeconds)
{
    try
                               ...
                               qint32 & rateLimitSeconds)
{
    ...
    try
    {
        syncChunk = m_pQecNoteStore->getFilteredSyncChunk(afterUSN, maxEntries,
                                                          filter);
        return 0;
    }
    catch(const qevercloud::EDAMUserException & userException)
    ...
    ErrorString & errorDescription, qint32 & rateLimitSeconds)
{
    try
    {
        syncState = m_pQecNoteStore->getLinkedNotebookSyncState(linkedNotebook,
                                                                authToken);
        return 0;
    }
    catch(const qevercloud::EDAMUserException & userException)
    ...
    ErrorString & errorDescription, qint32 & rateLimitSeconds)
{
    ...
    {
        syncChunk =
            m_pQecNoteStore->getLinkedNotebookSyncChunk(linkedNotebook, afterUSN,
        ...
        if (!notFoundException.exceptionData().isNull())
        {
            ...
            if (errorMessage == QStringLiteral("LinkedNotebook"))
            {
                errorDescription.appendBase(QT_TR_NOOP("the provided information "
                                                       "doesn't match any valid "
                                                       "notebook"));
            }
            else if (errorMessage == QStringLiteral("LinkedNotebook.uri"))
            {
                errorDescription.appendBase(QT_TR_NOOP("the provided public URI "
                                                       "doesn't match any valid "
                                                       "notebook"));
            }
                          ...
                          qint32 & rateLimitSeconds)
{
    ...
    try
    {
        note.qevercloudNote() = m_pQecNoteStore->getNote(note.guid(), withContent,
                                                         withResourcesData,
                                                         withResourcesRecognition,
                                                         withResourceAlternateData);
        note.setLocal(false);
    ...
    {
        return processEdamUserExceptionForGetNote(note.qevercloudNote(),
                                                  userException,
                                                  errorDescription);
    }
    catch(const qevercloud::EDAMNotFoundException & notFoundException)
...
bool NoteStore::getNoteAsync(const bool withContent,
...
qint32 NoteStore::getResource(const bool withDataBody,
                              ...
                              qint32 & rateLimitSeconds)
{
    ...
    if (!resource.hasGuid()) {
        errorDescription.setBase(QT_TR_NOOP("can't get resource: resource's "
                                            "guid is empty"));
        return qevercloud::EDAMErrorCode::UNKNOWN;
    }
    ...
    try
    {
        resource.qevercloudResource() =
            m_pQecNoteStore->getResource(resource.guid(), withDataBody,
                                         withRecognitionDataBody,
                                         withAttributes, withAlternateDataBody,
                                         authToken);
    ...
    {
        return processEdamUserExceptionForGetResource(resource.qevercloudResource(),
                                                      userException,
                                                      errorDescription);
    }
    catch(const qevercloud::EDAMNotFoundException & notFoundException)
...
bool NoteStore::getResourceAsync(const bool withDataBody,
                                 const bool withRecognitionDataBody,
                                 const bool withAlternateDataBody,
                                 const bool withAttributes,
                                 ...
                                 ErrorString & errorDescription)
{
    ...
    if (Q_UNLIKELY(resourceGuid.isEmpty())) {
        errorDescription.setBase(QT_TR_NOOP("Detected the attempt to get full "
                                            "resource's data for empty resource "
                                            "guid"));
        return false;
    }

    qevercloud::AsyncResult * pAsyncResult =
        m_pQecNoteStore->getResourceAsync(resourceGuid, withDataBody,
                                          withRecognitionDataBody,
                                          withAttributes, withAlternateDataBody,
                                          authToken);
    if (Q_UNLIKELY(!pAsyncResult)) {
        errorDescription.setBase(QT_TR_NOOP("Can't get full resource data: internal "
                                            "error, QEverCloud library returned "
                                            "null pointer to asynchronous result "
                                            "object"));
        return false;
    }
    ...
    ErrorString & errorDescription, qint32 & rateLimitSeconds)
{
    ...
    catch(const qevercloud::EDAMUserException & userException)
    {
        if (userException.errorCode == qevercloud::EDAMErrorCode::DATA_REQUIRED) {
            errorDescription.setBase(QT_TR_NOOP("no valid authentication token "
                                                "for current user"));
        }
        else if (userException.errorCode == qevercloud::EDAMErrorCode::PERMISSION_DENIED) {
            errorDescription.setBase(QT_TR_NOOP("share requires login, and another "
                                                "username has already been bound "
                                                "to this notebook"));
        }
        else {
    ...
    catch(const qevercloud::EDAMSystemException & systemException)
    {
        if (systemException.errorCode == qevercloud::EDAMErrorCode::RATE_LIMIT_REACHED)
        {
            ...
            {
                errorDescription.setBase(QT_TR_NOOP("QEverCloud error: "
    ...
    QVariant result, QSharedPointer<EverCloudExceptionData> exceptionData)
{
    ...
    if (!exceptionData.isNull())
    {
        ...
        {
            errorCode = processEdamUserExceptionForGetNote(note, userException,
                                                           errorDescription);
        }
        catch(const qevercloud::EDAMNotFoundException & notFoundException)
        {
        ...
        {
            errorCode = processEdamSystemException(systemException,
                                                   errorDescription,
                                                   rateLimitSeconds);
        }
    ...
    QVariant result, QSharedPointer<EverCloudExceptionData> exceptionData)
{
    ...
    if (!exceptionData.isNull())
    {
        ...
        {
            errorCode = processEdamUserExceptionForGetResource(resource,
                                                               userException,
                                                               errorDescription);
        }
        catch(const qevercloud::EDAMNotFoundException & notFoundException)
        ...
        Q_EMIT getResourceAsyncFinished(errorCode, resource, rateLimitSeconds,
                                        errorDescription);
        return;
    }
    ...
    ErrorString & errorDescription) const
{
    ...
    {
        if (thrownOnCreation) {
            errorDescription.setBase(QT_TR_NOOP("BAD_DATA_FORMAT exception during "
        ...
        {
            if (tag.hasName()) {
                errorDescription.appendBase(QT_TR_NOOP("invalid length or pattern "
                                                       "of tag's name"));
        ...
        else if (userException.parameter.ref() == QStringLiteral("Tag.parentGuid"))
        {
            if (tag.hasParentGuid()) {
                ...
                errorDescription.appendBase(QT_TR_NOOP("error code indicates "
                                                       "malformed parent guid "
                                                       "but it is empty"));
            }
        }
        else
    ...
    {
        if (thrownOnCreation) {
            errorDescription.setBase(QT_TR_NOOP("DATA_CONFLICT exception during "
                                                "the attempt to create a tag"));
        }
        else {
            errorDescription.setBase(QT_TR_NOOP("DATA_CONFLICT exception during "
        ...
        {
            if (tag.hasParentGuid()) {
                errorDescription.appendBase(QT_TR_NOOP("can't set parent for tag: "
    ...
    {
        errorDescription.setBase(QT_TR_NOOP("LIMIT_REACHED exception during "
                                            "the attempt to create a tag"));

        if (userException.parameter.isSet() &&
            (userException.parameter.ref() == QStringLiteral("Tag")))
        {
            errorDescription.appendBase(QT_TR_NOOP("already at max number of tags, "
    ...
    {
        errorDescription.setBase(QT_TR_NOOP("PERMISSION_DENIED exception during "
                                            "the attempt to update a tag"));

        if (userException.parameter.isSet() &&
            (userException.parameter.ref() == QStringLiteral("Tag")))
        {
            errorDescription.appendBase(QT_TR_NOOP("user doesn't own the tag, "
                                                   "it can't be updated"));
            if (tag.hasName()) {
                errorDescription.details() = tag.name();
            }
    ...
    return processUnexpectedEdamUserException(QStringLiteral("tag"), userException,
    ...
    ErrorString & errorDescription) const
{
    ...
    else if (userException.errorCode == qevercloud::EDAMErrorCode::DATA_CONFLICT)
    {
        ...
        {
            if (search.hasName()) {
                errorDescription.appendBase(QT_TR_NOOP("saved search's name is "
                                                       "already in use"));
                errorDescription.details() = search.name();
            }
            else {
    ...
    return processUnexpectedEdamUserException(QStringLiteral("saved search"),
                                              userException, source,
                                              errorDescription);
}
    ...
    ErrorString & errorDescription) const
{
    ...
    if (userException.errorCode == qevercloud::EDAMErrorCode::BAD_DATA_FORMAT)
    {
        ...
        else if (userException.parameter.ref() == QStringLiteral("Publishing.uri"))
        {
            if (notebook.hasPublishingUri()) {
                errorDescription.appendBase(QT_TR_NOOP("invalid publishing uri "
                                                       "for notebook"));
                errorDescription.details() = notebook.publishingUri();
            }
            else {
                errorDescription.appendBase(QT_TR_NOOP("notebook has no "
                                                       "publishing uri"));
            }
        }
                 ...
                 QStringLiteral("Publishing.publicDescription"))
        {
            if (notebook.hasPublishingPublicDescription()) {
                errorDescription.appendBase(QT_TR_NOOP("public description for "
                                                       "notebook is too long"));
                errorDescription.details() = notebook.publishingPublicDescription();
            }
            else {
                errorDescription.appendBase(QT_TR_NOOP("notebook has no public "
                                                       "description"));
            }
        }
        else
    ...
    else if (userException.errorCode == qevercloud::EDAMErrorCode::DATA_CONFLICT)
    {
        ...
        {
            if (notebook.hasName()) {
                errorDescription.appendBase(QT_TR_NOOP("notebook's name is "
                                                       "already in use"));
                errorDescription.details() = notebook.name();
            }
            else {
        ...
        {
            if (notebook.hasPublishingUri()) {
                errorDescription.appendBase(QT_TR_NOOP("notebook's publishing uri "
                                                       "is already in use"));
                errorDescription.details() = notebook.publishingUri();
            }
            else {
             ...
             (userException.errorCode == qevercloud::EDAMErrorCode::LIMIT_REACHED))
    {
        ...
        {
            errorDescription.appendBase(QT_TR_NOOP("already at max number of "
    ...
    return processUnexpectedEdamUserException(QStringLiteral("notebook"),
                                              userException, source,
                                              errorDescription);
}
    ...
    ErrorString & errorDescription) const
{
    ...
    else if (userException.errorCode == qevercloud::EDAMErrorCode::DATA_CONFLICT)
    {
        ...
        if (userException.parameter.ref() == QStringLiteral("Note.deleted")) {
            errorDescription.appendBase(QT_TR_NOOP("deletion timestamp is set "
                                                   "on active note"));
        }
        else {
            errorDescription.appendBase(QT_TR_NOOP("unexpected parameter"));
    ...
    {
        if (thrownOnCreation) {
            errorDescription.setBase(QT_TR_NOOP("DATA_REQUIRED exception during "
                                                "the attempt to create a note"));
        }
        else {
            errorDescription.setBase(QT_TR_NOOP("DATA_REQUIRED exception during "
    ...
    {
        if (thrownOnCreation) {
            errorDescription.setBase(QT_TR_NOOP("ENML_VALIDATION exception during "
                                                "the attempt to create a note"));
        }
        else {
            errorDescription.setBase(QT_TR_NOOP("ENML_VALIDATION exception during "
    ...
    else if (userException.errorCode == qevercloud::EDAMErrorCode::LIMIT_REACHED)
    {
        ...
        {
            errorDescription.appendBase(QT_TR_NOOP("already at maximum number "
    ...
    {
        if (thrownOnCreation) {
            errorDescription.setBase(QT_TR_NOOP("QUOTA_REACHED exception during "
                                                "the attempt to create a note"));
        }
        else {
            errorDescription.setBase(QT_TR_NOOP("QUOTA_REACHED exception during "
    ...
    return processUnexpectedEdamUserException(QStringLiteral("note"),
                                              userException, source,
                                              errorDescription);
}
    ...
    ErrorString & errorDescription) const
{
    ...
    if (thrownOnCreation) {
        errorDescription.setBase(QT_TR_NOOP("Unexpected EDAM user exception on "
    ...
    ErrorString & errorDescription, qint32 & rateLimitSeconds) const
{
    ...
    if (systemException.errorCode == qevercloud::EDAMErrorCode::RATE_LIMIT_REACHED)
    {
        if (!systemException.rateLimitDuration.isSet())
        {
            errorDescription.setBase(QT_TR_NOOP("Evernote API rate limit exceeded "
                                                "but no rate limit duration is "
                                                "available"));
        }
        else
        {
    ...
    ErrorString & errorDescription) const
{
    ...
    if (notFoundException.key.isSet() &&
        !notFoundException.key->isEmpty())
    {
