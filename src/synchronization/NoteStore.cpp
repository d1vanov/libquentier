    ...
    qint32 & rateLimitSeconds, const QString & linkedNotebookAuthToken)
{
    try
    {
        ...
        qevercloud::Note noteMetadata = m_pQecNoteStore->createNote(
    ...
    qint32 & rateLimitSeconds, const QString & linkedNotebookAuthToken)
{
    try
    {
        ...
        qevercloud::Note noteMetadata = m_pQecNoteStore->updateNote(
            note.qevercloudNote(), ctx);
    ...
    ErrorString & errorDescription, qint32 & rateLimitSeconds)
{
    ...
    catch(const qevercloud::EDAMNotFoundException & notFoundException)
    {
        ...
        if (notFoundException.exceptionData()) {
            errorDescription.details() += ToString(
    ...
    ErrorString & errorDescription)
{
    ...
    qevercloud::AsyncResult * pAsyncResult =
        m_pQecNoteStore->getNoteWithResultSpecAsync(
            noteGuid,
            noteResultSpec,
            ctx);
    ...
    ErrorString & errorDescription, qint32 & rateLimitSeconds)
{
    try
    {
        ...
        authResult = m_pQecNoteStore->authenticateToSharedNotebook(
            shareKey,
            ctx);
        return 0;
    }
    catch(const qevercloud::EDAMUserException & userException)
    {
        ...
        else if (userException.errorCode ==
    ...
    catch(const qevercloud::EDAMSystemException & systemException)
    {
        ...
        else if (systemException.errorCode ==
    ...
    ErrorString & errorDescription) const
{
    ...
    errorDescription.setBase(
        QT_TR_NOOP("Unexpected EDAM user exception on attempt to get resource"));
    errorDescription.details() = QStringLiteral("error code = ");
    errorDescription.details() += ToString(userException.errorCode);
    ...
    ErrorString & errorDescription) const
{
    ...
    if (userException.errorCode == qevercloud::EDAMErrorCode::BAD_DATA_FORMAT)
    {
        ...
        else if (userException.parameter.ref() == QStringLiteral("Notebook.stack"))
        {
            ...
            {
                errorDescription.appendBase(
                    QT_TR_NOOP("invalid length or pattern of notebook's stack"));
