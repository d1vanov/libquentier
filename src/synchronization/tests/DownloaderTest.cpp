...
TEST_F(DownloaderTest, CtorNullNoteStoreProvider)
{
    EXPECT_THROW(
        const auto downloader = std::make_shared<Downloader>(
            ...
            m_mockNotesProcessor, m_mockResourcesProcessor,
            m_mockSavedSearchesProcessor, m_mockTagsProcessor,
            m_mockFullSyncStaleDataExpunger, nullptr,
            m_mockLinkedNotebookTagsCleaner, m_mockLocalStorage, m_ctx,
            m_retryPolicy),
        InvalidArgument);
}
...
TEST_F(DownloaderTest, CtorNullLinkedNotebookTagsCleaner)
{
    EXPECT_THROW(
        const auto downloader = std::make_shared<Downloader>(
            ...
            m_mockNotesProcessor, m_mockResourcesProcessor,
            m_mockSavedSearchesProcessor, m_mockTagsProcessor,
            m_mockFullSyncStaleDataExpunger, m_mockNoteStoreProvider, nullptr,
            m_mockLocalStorage, m_ctx, m_retryPolicy),
        InvalidArgument);
}
...
TEST_F(DownloaderTest, CtorNullLocalStorage)
{
    EXPECT_THROW(
        const auto downloader = std::make_shared<Downloader>(
            ...
            m_mockSavedSearchesProcessor, m_mockTagsProcessor,
            m_mockFullSyncStaleDataExpunger, m_mockNoteStoreProvider,
            m_mockLinkedNotebookTagsCleaner, nullptr, m_ctx, m_retryPolicy),
        InvalidArgument);
}
...
TEST_F(DownloaderTest, CtorNullRequestContext)
{
    EXPECT_NO_THROW(
        const auto downloader = std::make_shared<Downloader>(
            ...
            m_mockSavedSearchesProcessor, m_mockTagsProcessor,
            m_mockFullSyncStaleDataExpunger, m_mockNoteStoreProvider,
            m_mockLinkedNotebookTagsCleaner, m_mockLocalStorage, nullptr,
            m_retryPolicy));
}
...
TEST_F(DownloaderTest, HandleNoUpdatesOnServerSide)
{
...

    {
        ...
        EXPECT_CALL(*m_mockNotesProcessor, cleanup);
        EXPECT_CALL(*m_mockResourcesProcessor, cleanup);
    }
...
TEST_P(DownloaderSyncChunksTest, Download)
{
...

    {
        ...
        EXPECT_CALL(
            *m_mockResourcesProcessor, processResources(syncChunks, _, _, _, _))
            .WillOnce(
                [&](const QList<qevercloud::SyncChunk> & chunks,
                    const utility::cancelers::ICancelerPtr & canceler,
                    [[maybe_unused]] const qevercloud::IRequestContextPtr & ctx,
                ...
                *m_mockNotesProcessor,
                ...
                .WillOnce(
                    [&](const QList<qevercloud::SyncChunk> & chunks,
                        ...
                        return threading::makeReadyFuture<
                            DownloadNotesStatusPtr>(
                            std::move(downloadNotesStatus));
                    });

            EXPECT_CALL(
                *m_mockResourcesProcessor,
                processResources(linkedNotebookSyncChunks, _, _, _, _))
                .WillOnce(
                    [&](const QList<qevercloud::SyncChunk> & chunks,
                        const utility::cancelers::ICancelerPtr & canceler,
                        [[maybe_unused]] const qevercloud::IRequestContextPtr &
    ...
    for (const auto & linkedNotebook: std::as_const(linkedNotebooks)) {
        EXPECT_CALL(
            *m_mockSyncChunksStorage,
            clearLinkedNotebookSyncChunks(*linkedNotebook.guid()));
    }
