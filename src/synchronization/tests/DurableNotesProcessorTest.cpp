    ...
    HandleDifferentCallbacksDuringSyncChunksProcessing)
{
    ...
    EXPECT_CALL(*m_mockNotesProcessor, processNotes)
        .WillOnce([&](const QList<qevercloud::SyncChunk> & syncChunks,
            ...
            status->m_processedNoteGuidsAndUsns
                [syncChunkNotes[0].guid().value()] =
    ...
    EXPECT_EQ(
        status->m_notesWhichFailedToProcess.constBegin()->first, notes[1]);
    ...
    ProcessSyncChunksWithPreviousSyncInfo)
{
    ...
    DownloadNotesStatusPtr previousExpungedNotesStatus;
    ...
    const DownloadNotesStatusPtr expectedStatus = [&] {
        auto expectedStatus = std::make_shared<DownloadNotesStatus>();
