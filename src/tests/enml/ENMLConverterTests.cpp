...
bool convertNoteToHtmlAndBackImpl(const QString & noteContent,
                                  DecryptedTextManager & decryptedTextManager,
                                  QString & error);
bool compareEnml(const QString & original,
...
bool convertNoteWithEncryptionToHtmlAndBack(QString & error)
{
    ...
    decryptedTextManager.addEntry(
        QStringLiteral("RU5DMBwXjfKR+x9ksjSJhtiF+CxfwXn2Hf/WqdVwLwJDX9YX5R34Z5SBMSCIOFF"
                       "r1MUeNkzHGVP5fHEppUlIExDG/Vpjh9KK1uu0VqTFoUWA0IXAAMA5eHnbxhBrjvL"
                       "3CoTQV7prRqJVLpUX77Q0vbNims1quxVWaf7+uVeK60YoiJnSOHvEYptoOs1FVfZ"
                                  ...
                                  QString & error)
{
    ...
    bool res = converter.noteContentToHtml(originalNoteContent, html, errorMessage,
    ...
    html.prepend(QStringLiteral("<!DOCTYPE HTML PUBLIC \"-//W3C//DTD HTML 4.01//EN\" "
...
bool compareEnml(const QString & original, const QString & processed,
                 QString & error)
{
...
#define PRINT_WARNING(err) \
    ...
    while(!readerOriginal.atEnd() && !readerProcessed.atEnd())
    {
        ...
        {
            error = QStringLiteral("QXmlStreamReader of the original ENML is "
        ...
        if (readerOriginal.isStartElement())
        {
            ...
            if (originalName != processedName) {
                error = QStringLiteral("Found a tag in the original ENML which "
                                       "name doesn't match the name of "
            ...
            if (originalName == QStringLiteral("en-todo"))
            {
                ...
                {
                    error = QStringLiteral("Checked state of ToDo item from "
                                           "the original ENML doesn't match "
                                           "the state of the item from "
                                           "the processed ENML");
                    PRINT_WARNING(error);
                    return false;
                }
            ...
            else if (originalName == QStringLiteral("td"))
            {
                ...
                for(int i = 0; i < numOriginalAttributes; ++i)
                {
                    ...
                    {
                        QNTRACE(QStringLiteral("Won't compare the style attribute "
                                               "for td tag as it's known to be "
                                               "slightly modified by the web engine "
                    ...
                    {
                        error = QStringLiteral("The corresponding attributes within "
...
bool convertHtmlWithModifiedDecryptedTextToEnml(QString & error)
{
    ...
    // re-calculate the hash of the encrypted text and ensure this entry is present
    // in decrypted text cache if this decryption needs to be remembered for
    // the whole session
    ...
    ENMLConverter::NoteContentToHtmlExtraData extraData;
    ErrorString errorMessage;
    bool res = converter.noteContentToHtml(originalENML, html, errorMessage,
                                           decryptedTextManager, extraData);
    if (!res) {
        error = QStringLiteral("Unable to convert the note content to HTML: ");
        error += errorMessage.nonLocalizedString();
    ...
    errorMessage.clear();
    res = converter.htmlToNoteContent(html, processedENML,
                                      decryptedTextManager, errorMessage);
    if (!res) {
        error = QStringLiteral("Unable to convert HTML to note content: ");
        error += errorMessage.nonLocalizedString();
...
bool convertHtmlWithTableHelperTagsToEnml(QString & error)
{
    QFile file(QStringLiteral(":/tests/complexNote3.txt"));
    if (!file.open(QIODevice::ReadOnly)) {
        error = QStringLiteral("Can't open the resource with complex note #3 "
                               "for reading");
        return false;
    }
    ...
    if (!file.open(QIODevice::ReadOnly)) {
        error = QStringLiteral("Can't open the resource with complex note html "
                               "with helper tags for reading");
        return false;
    }
    ...
    QString processedNoteContent;
    ErrorString errorMessage;
    bool res = converter.htmlToNoteContent(html, processedNoteContent,
                                           decryptedTextManager, errorMessage,
                                           skipRules);
    if (!res) {
        error = QStringLiteral("Unable to convert HTML to note content: ");
