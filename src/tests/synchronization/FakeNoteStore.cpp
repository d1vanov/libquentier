...
QHash<QString,qevercloud::SavedSearch> FakeNoteStore::savedSearches() const
{
    ...
    const SavedSearchDataByGuid & savedSearchByGuid =
        m_data->m_savedSearches.get<SavedSearchByGuid>();
    for(auto it = savedSearchByGuid.begin(),
        end = savedSearchByGuid.end(); it != end; ++it)
    {
...
bool FakeNoteStore::setSavedSearch(SavedSearch & search, ErrorString & errorDescription)
{
    ...
    SavedSearchDataByNameUpper & nameIndex =
    ...
    SavedSearchDataByGuid & savedSearchGuidIndex =
        m_data->m_savedSearches.get<SavedSearchByGuid>();
...
{
    const SavedSearchDataByGuid & index =
        m_data->m_savedSearches.get<SavedSearchByGuid>();
...
QHash<QString,qevercloud::Tag> FakeNoteStore::tags() const
{
    ...
    for(auto it = tagDataByGuid.begin(),
        end = tagDataByGuid.end(); it != end; ++it)
    {
...
bool FakeNoteStore::setTag(Tag & tag, ErrorString & errorDescription)
{
    ...
    if (tag.hasLinkedNotebookGuid())
    {
        ...
        const LinkedNotebookDataByGuid & index =
            m_data->m_linkedNotebooks.get<LinkedNotebookByGuid>();
        auto it = index.find(linkedNotebookGuid);
        if (it == index.end()) {
            errorDescription.setBase(QStringLiteral("Can't set tag with linked "
                                                    "notebook guid corresponding "
    ...
    {
        qint32 maxUsn = currentMaxUsn(tag.hasLinkedNotebookGuid()
...
bool FakeNoteStore::removeTag(const QString & guid)
{
    ...
    const TagDataByParentTagGuid & parentTagGuidIndex =
        m_data->m_tags.get<TagByParentTagGuid>();
    ...
    for(auto it = childTagGuids.constBegin(),
        end = childTagGuids.constEnd(); it != end; ++it)
    {
    ...
    if (removedChildTags)
    {
        ...
        if (Q_UNLIKELY(tagIt == index.end())) {
            QNWARNING(QStringLiteral("Tag to be removed is not not found after ")
                      << QStringLiteral("the removal of its child tags: guid = ")
                      << guid);
            return false;
        }
    }
    ...
    for(auto noteIt = noteGuidIndex.begin();
        noteIt != noteGuidIndex.end(); ++noteIt)
    {
...
QHash<QString,qevercloud::Notebook> FakeNoteStore::notebooks() const
{
    ...
    const NotebookDataByGuid & notebookDataByGuid =
        m_data->m_notebooks.get<NotebookByGuid>();
    for(auto it = notebookDataByGuid.begin(),
        end = notebookDataByGuid.end(); it != end; ++it)
    {
...
bool FakeNoteStore::setNotebook(Notebook & notebook, ErrorString & errorDescription)
{
    ...
    if (notebook.hasLinkedNotebookGuid())
    {
        ...
        auto it = index.find(linkedNotebookGuid);
        if (it == index.end()) {
            errorDescription.setBase(QStringLiteral("Can't set notebook with "
                                                    "linked notebook guid "
                                                    "corresponding to no existing "
                                                    "linked notebook"));
            return false;
        }
    }
    ...
    NotebookDataByNameUpper & nameIndex =
        m_data->m_notebooks.get<NotebookByNameUpper>();
    ...
    {
        qint32 maxUsn = currentMaxUsn(notebook.hasLinkedNotebookGuid()
    ...
    NotebookDataByGuid & notebookGuidIndex =
        m_data->m_notebooks.get<NotebookByGuid>();
...
bool FakeNoteStore::removeNotebook(const QString & guid)
{
    ...
    const NoteDataByNotebookGuid & noteDataByNotebookGuid =
    ...
    for(auto it = noteGuids.constBegin(),
        end = noteGuids.constEnd(); it != end; ++it)
    {
        removeNote(*it);
    }
...
{
    const NotebookDataByLinkedNotebookGuid & index =
...
QHash<QString,qevercloud::Note> FakeNoteStore::notes() const
{
    ...
    for(auto it = noteDataByGuid.begin(),
        end = noteDataByGuid.end(); it != end; ++it)
    {
...
bool FakeNoteStore::setNote(Note & note, ErrorString & errorDescription)
{
    ...
    if (notebookIt == notebookGuidIndex.end()) {
        errorDescription.setBase(QStringLiteral("Can't set note: no notebook "
                                                "was found for it by guid"));
        return false;
    }

    if (!note.hasUpdateSequenceNumber())
    {
        qint32 maxUsn = currentMaxUsn(notebookIt->hasLinkedNotebookGuid()
...
bool FakeNoteStore::removeNote(const QString & guid)
{
    ...
    {
        QList<Resource> resources = note.resources();
        for(auto resIt = resources.constBegin(),
            resEnd = resources.constEnd(); resIt != resEnd; ++resIt)
        {
...
{
    const NoteDataByConflictSourceNoteGuid & index =
        m_data->m_notes.get<NoteByConflictSourceNoteGuid>();
...
QHash<QString,qevercloud::Resource> FakeNoteStore::resources() const
{
    ...
    const ResourceDataByGuid & resourceDataByGuid =
        m_data->m_resources.get<ResourceByGuid>();
    for(auto it = resourceDataByGuid.begin(),
        end = resourceDataByGuid.end(); it != end; ++it)
    {
...
bool FakeNoteStore::setResource(Resource & resource, ErrorString & errorDescription)
{
    ...
    if (noteIt == noteGuidIndex.end()) {
        errorDescription.setBase(QStringLiteral("Can't set resource: no note "
                                                "was found for it by guid"));
        return false;
    }
    ...
    if (notebookIt == notebookGuidIndex.end()) {
        errorDescription.setBase(QStringLiteral("Can't set resource: no notebook "
                                                "was found for resource's note "
                                                "by notebook guid"));
        return false;
    }
    ...
    ResourceDataByGuid & resourceGuidIndex =
        m_data->m_resources.get<ResourceByGuid>();
...
bool FakeNoteStore::removeResource(const QString & guid)
{
    ...
    if (noteIt != noteGuidIndex.end()) {
        ...
        QNWARNING(QStringLiteral("Found no note corresponding to the removed resource: ")
                  << *it);
    }
...
QHash<QString,qevercloud::LinkedNotebook> FakeNoteStore::linkedNotebooks() const
{
    ...
    const LinkedNotebookDataByGuid & linkedNotebookDataByGuid =
        m_data->m_linkedNotebooks.get<LinkedNotebookByGuid>();
    for(auto it = linkedNotebookDataByGuid.begin(),
        end = linkedNotebookDataByGuid.end(); it != end; ++it)
    {
...
bool FakeNoteStore::setLinkedNotebook(LinkedNotebook & linkedNotebook,
                                      ErrorString & errorDescription)
{
    if (!linkedNotebook.hasGuid()) {
        errorDescription.setBase(QStringLiteral("Can't set linked notebook "
                                                "without guid"));
        return false;
    }
    ...
    if (!linkedNotebook.hasShardId() && !linkedNotebook.hasUri()) {
        errorDescription.setBase(QStringLiteral("Can't set linked notebook without "
                                                "either shard id or uri"));
        return false;
    }
    ...
    LinkedNotebookDataByGuid & index =
        m_data->m_linkedNotebooks.get<LinkedNotebookByGuid>();
...
void FakeNoteStore::setLinkedNotebookSyncState(const QString & linkedNotebookOwner,
...
void FakeNoteStore::setLinkedNotebookAuthToken(const QString & linkedNotebookOwner,
                                               const QString & linkedNotebookAuthToken)
{
    QNDEBUG(QStringLiteral("FakeNoteStore::setLinkedNotebookAuthToken: owner = ")
            << linkedNotebookOwner << QStringLiteral(", token = ")
...
{
    QNDEBUG(QStringLiteral("FakeNoteStore::currentMaxUsn: linked notebook guid = ")
            << linkedNotebookGuid);
    ...
    if (linkedNotebookGuid.isEmpty() && !savedSearchUsnIndex.empty())
    {
        ...
        if (lastSavedSearchIt->updateSequenceNumber() > maxUsn) {
            ...
            QNTRACE(QStringLiteral("Updated max USN to ") << maxUsn
                    << QStringLiteral(" from saved search: ") << *lastSavedSearchIt);
        }
    }
    ...
    if (!tagUsnIndex.empty())
    {
            ...
            tagCounter < numTags; ++tagCounter, --tagIt)
        {
            ...
            if (!matchesByLinkedNotebook) {
                QNTRACE(QStringLiteral("Skipping tag ") << *tagIt
            ...
            if (tagIt->updateSequenceNumber() > maxUsn) {
                ...
                QNTRACE(QStringLiteral("Updated max USN to ") << maxUsn
                        << QStringLiteral(" from tag: ") << *tagIt);
            }
            else {
                QNTRACE(QStringLiteral("Skipping tag ") << *tagIt
                        << QStringLiteral("\nAs its USN is no greater than ")
                        << QStringLiteral("max USN ") << maxUsn);
            }
        }
    }

    const NotebookDataByUSN & notebookUsnIndex =
        m_data->m_notebooks.get<NotebookByUSN>();
    if (!notebookUsnIndex.empty())
    {
            ...
            notebookCounter < numNotebooks; ++notebookCounter, --notebookIt)
        {
            ...
            bool matchesByLinkedNotebook =
                ( ( !linkedNotebookGuid.isEmpty() &&
                  ...
                  (linkedNotebookGuid.isEmpty() &&
                   !notebookIt->hasLinkedNotebookGuid()) );
            if (!matchesByLinkedNotebook) {
                QNTRACE(QStringLiteral("Skipping notebook ") << *notebookIt
                        << QStringLiteral("\nAs it doesn't match by linked notebook"));
                continue;
            }
            ...
            if (notebookIt->updateSequenceNumber() > maxUsn) {
                ...
                QNTRACE(QStringLiteral("Updated max USN to ") << maxUsn
                        << QStringLiteral(" from notebook: ") << *notebookIt);
            }
            else {
                QNTRACE(QStringLiteral("Skipping notebook ") << *notebookIt
    ...
    if (!noteUsnIndex.empty())
    {
            ...
            noteCounter < numNotes; ++noteCounter, --noteIt)
        {
            ...
            if (!matchesByLinkedNotebook) {
                QNTRACE(QStringLiteral("Skipping note ") << *noteIt
                        << QStringLiteral("\nAs it doesn't match by linked notebook"));
                continue;
            }
            ...
            if (noteIt->updateSequenceNumber() > maxUsn) {
                ...
                QNTRACE(QStringLiteral("Updated max USN to ") << maxUsn
                        << QStringLiteral(" from note: ") << *noteIt);
            }
            else {
                QNTRACE(QStringLiteral("Skipping note ") << *noteIt
                        << QStringLiteral("\nAs its USN is no greater than max USN ")
    ...
    if (!resourceUsnIndex.empty())
    {
            ...
            resourceCounter < numResources; ++resourceCounter, --resourceIt)
        {
            ...
            if (!matchesByLinkedNotebook) {
                QNTRACE(QStringLiteral("Skipping resource ") << *resourceIt
                        << QStringLiteral("\nAs it doesn't match by linked notebook"));
                continue;
            }
            ...
            if (resourceIt->updateSequenceNumber() > maxUsn) {
                ...
                QNTRACE(QStringLiteral("Updated max USN to ") << maxUsn
                        << QStringLiteral(" from resource: ") << *resourceIt);
            }
            else {
                QNTRACE(QStringLiteral("Skipping resource ") << *resourceIt
                        << QStringLiteral("\nAs its USN is no greater than max USN ")
    ...
    const LinkedNotebookDataByUSN & linkedNotebookUsnIndex =
    ...
    if (linkedNotebookGuid.isEmpty() && !linkedNotebookUsnIndex.empty())
    {
        ...
        if (lastLinkedNotebookIt->updateSequenceNumber() > maxUsn) {
            ...
            QNTRACE(QStringLiteral("Updated max USN from linked notebook to ")
                    << maxUsn);
        }
    }
...
FakeNoteStore::WhenToTriggerAPIRateLimitsExceeding::type
FakeNoteStore::whenToTriggerAPIRateLimitsExceeding() const
{
...
qint32 FakeNoteStore::smallestUsnOfNotCompletelySentDataItemBeforeRateLimitBreach(
    const QString & linkedNotebookGuid) const
{
    QNDEBUG(QStringLiteral("FakeNoteStore::")
            << QStringLiteral("smallestUsnOfNotCompletelySentDataItemBeforeRateLimitBreach: ")
    ...
    const GuidsOfCompleteSentItems & guidsOfCompleteSentItems =
        (linkedNotebookGuid.isEmpty()
         ? m_data->m_guidsOfUserOwnCompleteSentItems
         : git.value());
    ...
    if (linkedNotebookGuid.isEmpty())
    {
        const SavedSearchDataByUSN & savedSearchUsnIndex =
            m_data->m_savedSearches.get<SavedSearchByUSN>();
        for(auto it = savedSearchUsnIndex.begin(),
            end = savedSearchUsnIndex.end(); it != end; ++it)
        {
            ...
            {
                QNTRACE(QStringLiteral("Encountered first notebook not within "
                ...
                {
                    smallestUsn = savedSearch.updateSequenceNumber();
                    QNTRACE(QStringLiteral("Updated smallest USN to ")
                            << smallestUsn);
                }
    ...
    const NotebookDataByUSN & notebookUsnIndex =
        m_data->m_notebooks.get<NotebookByUSN>();
    for(auto it = notebookUsnIndex.begin(),
        end = notebookUsnIndex.end(); it != end; ++it)
    {
        const Notebook & notebook = *it;
        if (linkedNotebookGuid.isEmpty() == notebook.hasLinkedNotebookGuid()) {
            QNTRACE(QStringLiteral("Skipping notebook not matching by linked ")
                    << QStringLiteral("notebook criteria: ") << notebook);
            continue;
        }
        if (notebook.hasLinkedNotebookGuid() &&
    ...
    for(auto it = tagUsnIndex.begin(), end = tagUsnIndex.end(); it != end; ++it)
    {
        ...
        if (linkedNotebookGuid.isEmpty() == tag.hasLinkedNotebookGuid()) {
            QNTRACE(QStringLiteral("Skipping tag not matching by linked notebook ")
                    << QStringLiteral("criteria: ") << tag);
            continue;
        }
        if (tag.hasLinkedNotebookGuid() &&
        ...
        {
            QNTRACE(QStringLiteral("Encountered first tag not within the list of "
                                   "sent items"));
    ...
    if (linkedNotebookGuid.isEmpty())
    {
        const LinkedNotebookDataByUSN & linkedNotebookUsnIndex =
            m_data->m_linkedNotebooks.get<LinkedNotebookByUSN>();
        for(auto it = linkedNotebookUsnIndex.begin(),
            end = linkedNotebookUsnIndex.end(); it != end; ++it)
        {
            const LinkedNotebook & linkedNotebook = *it;
            QNTRACE(QStringLiteral("Processing linked notebook: ") << linkedNotebook);

            auto guidIt =
                guidsOfCompleteSentItems.m_linkedNotebookGuids.find(linkedNotebook.guid());
            if (guidIt == guidsOfCompleteSentItems.m_linkedNotebookGuids.end())
            {
                QNTRACE(QStringLiteral("Encountered first linked notebook not "
    ...
    for(auto it = noteUsnIndex.begin(), end = noteUsnIndex.end(); it != end; ++it)
    {
        ...
        if (linkedNotebookGuid.isEmpty() == notebook.hasLinkedNotebookGuid()) {
            QNTRACE(QStringLiteral("Skipping note as its notebook doesn't match ")
                    << QStringLiteral("by linked notebook criteria: ") << note
                    << QStringLiteral("\nNotebook: ") << notebook);
            continue;
        }
        ...
        {
            QNTRACE(QStringLiteral("Encountered first note not within the list "
                                   "of sent items"));
    ...
    const ResourceDataByUSN & resourceUsnIndex =
        m_data->m_resources.get<ResourceByUSN>();
    const NoteDataByGuid & noteGuidIndex = m_data->m_notes.get<NoteByGuid>();
    for(auto it = resourceUsnIndex.begin(),
        end = resourceUsnIndex.end(); it != end; ++it)
    {
        const Resource & resource = *it;
        auto noteIt = noteGuidIndex.find(resource.noteGuid());
        ...
        auto notebookIt = notebookGuidIndex.find(note.notebookGuid());
        if (Q_UNLIKELY(notebookIt == notebookGuidIndex.end())) {
            QNWARNING(QStringLiteral("Skipping resource for which note ")
                      << QStringLiteral("no notebook was found: ") << note);
            continue;
        }
        ...
        if (linkedNotebookGuid.isEmpty() == notebook.hasLinkedNotebookGuid())
        {
            QNTRACE(QStringLiteral("Skipping resource as its note's notebook ")
                    << QStringLiteral("doesn't match by linked notebook criteria: ")
        ...
        {
            QNTRACE(QStringLiteral("Encountered first resource not within "
                                   "the list of sent items"));
    ...
    QNDEBUG(QStringLiteral("Smallest USN of not completely sent data item is ")
            << smallestUsn << QStringLiteral(" (linked notebook guid = ")
...
{
    for(auto it = m_data->m_getNoteAsyncDelayTimerIds.begin(),
        end = m_data->m_getNoteAsyncDelayTimerIds.end(); it != end; ++it)
    {
        killTimer(*it);
    }
    m_data->m_getNoteAsyncDelayTimerIds.clear();

    for(auto it = m_data->m_getResourceAsyncDelayTimerIds.begin(),
        end = m_data->m_getResourceAsyncDelayTimerIds.end(); it != end; ++it)
    {
        killTimer(*it);
    }
...
qint32 FakeNoteStore::createNotebook(Notebook & notebook,
                                     ErrorString & errorDescription,
                                     qint32 & rateLimitSeconds,
                                     const QString & linkedNotebookAuthToken)
{
    if (m_data->m_whenToTriggerAPIRateLimitExceeding ==
        WhenToTriggerAPIRateLimitsExceeding::OnCreateNotebookAttempt)
    {
        m_data->m_whenToTriggerAPIRateLimitExceeding =
            WhenToTriggerAPIRateLimitsExceeding::Never;
        rateLimitSeconds = 0;
    ...
    if (notebook.hasLinkedNotebookGuid())
    {
        checkRes = checkLinkedNotebookAuthTokenForNotebook(notebook.guid(),
                                                           linkedNotebookAuthToken,
                                                           errorDescription);
        if (checkRes != 0) {
            return checkRes;
    ...
    else if (!linkedNotebookAuthToken.isEmpty() &&
    ...
    if (!linkedNotebookAuthToken.isEmpty() && notebook.isDefaultNotebook()) {
        errorDescription.setBase(QStringLiteral("Linked notebook cannot be set "
                                                "as default notebook"));
    ...
    if (nameIt != nameIndex.end()) {
        errorDescription.setBase(QStringLiteral("Notebook with the specified "
                                                "name already exists"));
...
qint32 FakeNoteStore::updateNotebook(Notebook & notebook,
                                     ErrorString & errorDescription,
                                     qint32 & rateLimitSeconds,
                                     const QString & linkedNotebookAuthToken)
{
    if (m_data->m_whenToTriggerAPIRateLimitExceeding ==
        WhenToTriggerAPIRateLimitsExceeding::OnUpdateNotebookAttempt)
    {
        m_data->m_whenToTriggerAPIRateLimitExceeding =
            WhenToTriggerAPIRateLimitsExceeding::Never;
        rateLimitSeconds = 0;
...
qint32 FakeNoteStore::createNote(Note & note, ErrorString & errorDescription,
                                 qint32 & rateLimitSeconds,
                                 const QString & linkedNotebookAuthToken)
{
    if (m_data->m_whenToTriggerAPIRateLimitExceeding ==
        WhenToTriggerAPIRateLimitsExceeding::OnCreateNoteAttempt)
    {
        m_data->m_whenToTriggerAPIRateLimitExceeding =
            WhenToTriggerAPIRateLimitsExceeding::Never;
        rateLimitSeconds = 0;
    ...
    {
        checkRes = checkLinkedNotebookAuthTokenForNotebook(note.notebookGuid(),
                                                           linkedNotebookAuthToken,
                                                           errorDescription);
        if (checkRes != 0) {
            return checkRes;
             ...
             (linkedNotebookAuthToken != m_data->m_authenticationToken))
    {
        errorDescription.setBase(QStringLiteral("Note's notebook doesn't belong "
...
qint32 FakeNoteStore::updateNote(Note & note, ErrorString & errorDescription,
                                 qint32 & rateLimitSeconds,
                                 const QString & linkedNotebookAuthToken)
{
    if (m_data->m_whenToTriggerAPIRateLimitExceeding ==
        WhenToTriggerAPIRateLimitsExceeding::OnUpdateNoteAttempt)
    {
        m_data->m_whenToTriggerAPIRateLimitExceeding =
            WhenToTriggerAPIRateLimitsExceeding::Never;
        rateLimitSeconds = 0;
    ...
    if (it == index.end()) {
        errorDescription.setBase(QStringLiteral("Note with the specified guid "
                                                "doesn't exist"));
        return qevercloud::EDAMErrorCode::DATA_CONFLICT;
    }
    ...
    if (note.hasResources())
    {
        ...
        for(auto resIt = resources.begin(),
            resEnd = resources.end(); resIt != resEnd; ++resIt)
        {
...
qint32 FakeNoteStore::createTag(Tag & tag, ErrorString & errorDescription,
                                qint32 & rateLimitSeconds,
                                const QString & linkedNotebookAuthToken)
{
    if (m_data->m_whenToTriggerAPIRateLimitExceeding ==
        WhenToTriggerAPIRateLimitsExceeding::OnCreateTagAttempt)
    {
        m_data->m_whenToTriggerAPIRateLimitExceeding =
            WhenToTriggerAPIRateLimitsExceeding::Never;
        rateLimitSeconds = 0;
    ...
    if (tag.hasLinkedNotebookGuid())
    {
        checkRes = checkLinkedNotebookAuthTokenForTag(tag, linkedNotebookAuthToken,
                                                      errorDescription);
        if (checkRes != 0) {
            return checkRes;
        }
             ...
             (linkedNotebookAuthToken != m_data->m_authenticationToken))
    {
        errorDescription.setBase(QStringLiteral("Tag doesn't belong to a linked "
...
qint32 FakeNoteStore::updateTag(Tag & tag, ErrorString & errorDescription,
                                qint32 & rateLimitSeconds,
                                const QString & linkedNotebookAuthToken)
{
    if (m_data->m_whenToTriggerAPIRateLimitExceeding ==
        WhenToTriggerAPIRateLimitsExceeding::OnUpdateTagAttempt)
    {
        m_data->m_whenToTriggerAPIRateLimitExceeding =
            WhenToTriggerAPIRateLimitsExceeding::Never;
        rateLimitSeconds = 0;
    ...
    if (it == index.end()) {
        errorDescription.setBase(QStringLiteral("Tag with the specified guid "
                                                "doesn't exist"));
        return qevercloud::EDAMErrorCode::DATA_CONFLICT;
    }
    ...
    if (originalTag.name().toUpper() != tag.name().toUpper())
    {
        ...
        auto nameIt = nameIndex.find(tag.name().toUpper());
        if (nameIt != nameIndex.end()) {
            errorDescription.setBase(QStringLiteral("Tag with the specified name "
                                                    "already exists"));
            return qevercloud::EDAMErrorCode::DATA_CONFLICT;
        }
    }
...
qint32 FakeNoteStore::createSavedSearch(SavedSearch & savedSearch,
                                        ErrorString & errorDescription,
                                        qint32 & rateLimitSeconds)
{
    if (m_data->m_whenToTriggerAPIRateLimitExceeding ==
        WhenToTriggerAPIRateLimitsExceeding::OnCreateSavedSearchAttempt)
    {
        m_data->m_whenToTriggerAPIRateLimitExceeding =
            WhenToTriggerAPIRateLimitsExceeding::Never;
        rateLimitSeconds = 0;
    ...
    if (m_data->m_savedSearches.size() + 1 > m_data->m_maxNumSavedSearches) {
        errorDescription.setBase(QStringLiteral("Already at max number of "
                                                "saved searches"));
        return qevercloud::EDAMErrorCode::LIMIT_REACHED;
    }
    ...
    if (!m_data->m_onceAPIRateLimitExceedingTriggered) {
        Q_UNUSED(m_data->m_guidsOfUserOwnCompleteSentItems.m_savedSearchGuids.insert(
                savedSearch.guid()))
    }
...
qint32 FakeNoteStore::updateSavedSearch(SavedSearch & savedSearch,
                                        ErrorString & errorDescription,
                                        qint32 & rateLimitSeconds)
{
    if (m_data->m_whenToTriggerAPIRateLimitExceeding ==
        WhenToTriggerAPIRateLimitsExceeding::OnUpdateSavedSearchAttempt)
    {
        m_data->m_whenToTriggerAPIRateLimitExceeding =
            WhenToTriggerAPIRateLimitsExceeding::Never;
        rateLimitSeconds = 0;
    ...
    if (it == index.end()) {
        errorDescription.setBase(QStringLiteral("Saved search with the specified "
                                                "guid doesn't exist"));
        return qevercloud::EDAMErrorCode::DATA_CONFLICT;
    }
    ...
    {
        const SavedSearchDataByNameUpper & nameIndex =
            m_data->m_savedSearches.get<SavedSearchByNameUpper>();
...
qint32 FakeNoteStore::getSyncState(qevercloud::SyncState & syncState,
                                   ErrorString & errorDescription,
                                   qint32 & rateLimitSeconds)
{
    if (m_data->m_whenToTriggerAPIRateLimitExceeding ==
        WhenToTriggerAPIRateLimitsExceeding::OnGetUserOwnSyncStateAttempt)
    {
        m_data->m_whenToTriggerAPIRateLimitExceeding =
            WhenToTriggerAPIRateLimitsExceeding::Never;
        rateLimitSeconds = 0;
...
qint32 FakeNoteStore::getSyncChunk(const qint32 afterUSN, const qint32 maxEntries,
                                   const qevercloud::SyncChunkFilter & filter,
                                   qevercloud::SyncChunk & syncChunk,
                                   ErrorString & errorDescription,
                                   qint32 & rateLimitSeconds)
{
    QNDEBUG(QStringLiteral("FakeNoteStore::getSyncChunk: after USN = ") << afterUSN
            << QStringLiteral(", max entries = ") << maxEntries
            << QStringLiteral(", filter = ") << filter);

    if (m_data->m_whenToTriggerAPIRateLimitExceeding ==
        WhenToTriggerAPIRateLimitsExceeding::OnGetUserOwnSyncChunkAttempt)
    {
        m_data->m_whenToTriggerAPIRateLimitExceeding =
            WhenToTriggerAPIRateLimitsExceeding::Never;
        rateLimitSeconds = 0;
    ...
    return getSyncChunkImpl(afterUSN, maxEntries, (afterUSN == 0), QString(),
                            filter, syncChunk, errorDescription);
}
...
{
    if (m_data->m_whenToTriggerAPIRateLimitExceeding ==
        WhenToTriggerAPIRateLimitsExceeding::OnGetLinkedNotebookSyncStateAttempt)
    {
        m_data->m_whenToTriggerAPIRateLimitExceeding =
            WhenToTriggerAPIRateLimitsExceeding::Never;
        rateLimitSeconds = 0;
    ...
    if (it == m_data->m_linkedNotebookSyncStates.end())
    {
        QNWARNING(QStringLiteral("Failed to find linked notebook sync state for ")
                  << QStringLiteral("linked notebook: ") << linkedNotebook
                  << QStringLiteral("\nLinked notebook sync states: ")
                  << m_data->m_linkedNotebookSyncStates);
        errorDescription.setBase(QStringLiteral("Found no sync state for the given "
...
qint32 FakeNoteStore::getLinkedNotebookSyncChunk(
    ...
    qint32 & rateLimitSeconds)
{
    QNDEBUG(QStringLiteral("FakeNoteStore::getLinkedNotebookSyncChunk: linked ")
            << QStringLiteral("notebook = ") << linkedNotebook
            << QStringLiteral("\nAfter USN = ") << afterUSN
            << QStringLiteral(", max entries = ") << maxEntries
            << QStringLiteral(", linked notebook auth token = ")
            << linkedNotebookAuthToken << QStringLiteral(", full sync only = ")
            << (fullSyncOnly ? QStringLiteral("true") : QStringLiteral("false")));

    if (m_data->m_whenToTriggerAPIRateLimitExceeding ==
        WhenToTriggerAPIRateLimitsExceeding::OnGetLinkedNotebookSyncChunkAttempt)
    {
        m_data->m_whenToTriggerAPIRateLimitExceeding =
            WhenToTriggerAPIRateLimitsExceeding::Never;
        rateLimitSeconds = 0;
    ...
    const LinkedNotebookDataByUsername & linkedNotebookUsernameIndex =
    ...
    if (linkedNotebookIt == linkedNotebookUsernameIndex.end()) {
        errorDescription.setBase(QStringLiteral("Found no existing linked notebook "
                                                "by username"));
        return qevercloud::EDAMErrorCode::UNKNOWN;
    }
    ...
    return getSyncChunkImpl(afterUSN, maxEntries, fullSyncOnly,
                            linkedNotebook.guid.ref(), filter,
                              ...
                              qint32 & rateLimitSeconds)
{
    ...
    {
        if (m_data->m_whenToTriggerAPIRateLimitExceeding ==
            WhenToTriggerAPIRateLimitsExceeding::OnGetNoteAttemptAfterDownloadingLinkedNotebookSyncChunks)
        {
            m_data->m_whenToTriggerAPIRateLimitExceeding =
                WhenToTriggerAPIRateLimitsExceeding::Never;
            rateLimitSeconds = 0;
    ...
    {
        if (m_data->m_whenToTriggerAPIRateLimitExceeding ==
            WhenToTriggerAPIRateLimitsExceeding::OnGetNoteAttemptAfterDownloadingUserOwnSyncChunks)
        {
            m_data->m_whenToTriggerAPIRateLimitExceeding =
                WhenToTriggerAPIRateLimitsExceeding::Never;
            rateLimitSeconds = 0;
...
bool FakeNoteStore::getNoteAsync(const bool withContent,
                                 ...
                                 ErrorString & errorDescription)
{
    ...
    QNDEBUG(QStringLiteral("Started timer to postpone the get note result, ")
            << QStringLiteral("timer id = ") << timerId);

    Q_UNUSED(m_data->m_getNoteAsyncDelayTimerIds.insert(timerId))
    return true;
}

qint32 FakeNoteStore::getResource(const bool withDataBody,
                                  ...
                                  qint32 & rateLimitSeconds)
{
    ...
    {
        if (m_data->m_whenToTriggerAPIRateLimitExceeding ==
            WhenToTriggerAPIRateLimitsExceeding::OnGetResourceAttemptAfterDownloadingLinkedNotebookSyncChunks)
        {
            m_data->m_whenToTriggerAPIRateLimitExceeding =
                WhenToTriggerAPIRateLimitsExceeding::Never;
            rateLimitSeconds = 0;
    ...
    {
        if (m_data->m_whenToTriggerAPIRateLimitExceeding ==
            WhenToTriggerAPIRateLimitsExceeding::OnGetResourceAttemptAfterDownloadingUserOwnSyncChunks)
        {
            m_data->m_whenToTriggerAPIRateLimitExceeding =
                WhenToTriggerAPIRateLimitsExceeding::Never;
            rateLimitSeconds = 0;
    ...
    auto noteIt = noteGuidIndex.find(noteGuid);
    if (Q_UNLIKELY(noteIt == noteGuidIndex.end())) {
        errorDescription.setBase(QStringLiteral("Found no note containing "
                                                "the resource"));
        return qevercloud::EDAMErrorCode::INTERNAL_ERROR;
    }
    ...
    if (Q_UNLIKELY(notebookIt == notebookGuidIndex.end())) {
        errorDescription.setBase(QStringLiteral("Found no notebook containing "
    ...
    if (notebookIt->hasLinkedNotebookGuid())
    {
        qint32 checkRes = checkLinkedNotebookAuthTokenForNotebook(notebookGuid,
                                                                  authToken,
                                                                  errorDescription);
        if (checkRes != 0) {
            return checkRes;
...
bool FakeNoteStore::getResourceAsync(const bool withDataBody,
                                     const bool withRecognitionDataBody,
                                     const bool withAlternateDataBody,
                                     const bool withAttributes,
                                     ...
                                     ErrorString & errorDescription)
{
    ...
    int timerId = startTimer(0);
    QNDEBUG(QStringLiteral("Started timer to postpone the get resource result, ")
            << QStringLiteral("timer id = ") << timerId);
...
{
    if (m_data->m_whenToTriggerAPIRateLimitExceeding ==
        WhenToTriggerAPIRateLimitsExceeding::OnAuthenticateToSharedNotebookAttempt)
    {
        m_data->m_whenToTriggerAPIRateLimitExceeding =
            WhenToTriggerAPIRateLimitsExceeding::Never;
        rateLimitSeconds = 0;
    ...
    if (authTokenIt == m_data->m_linkedNotebookAuthTokens.end()) {
        errorDescription.setBase(QStringLiteral("No valid authentication token "
                                                "was provided"));
        return qevercloud::EDAMErrorCode::INVALID_AUTH;
    }
...
void FakeNoteStore::timerEvent(QTimerEvent * pEvent)
{
    ...
    {
        QNDEBUG(QStringLiteral("getNoteAsync delay timer event, timer id = ")
                << pEvent->timerId());
        ...
        if (!m_data->m_getNoteAsyncRequests.isEmpty())
        {
            ...
            qint32 res = getNote(request.m_withContent, request.m_withResourcesData,
                                 ...
                                 request.m_withResourcesAlternateData,
                                 note, errorDescription, rateLimitSeconds);
            if ((res == 0) && (note.hasNotebookGuid()))
            {
                const Notebook * pNotebook = findNotebook(note.notebookGuid());
                if (Q_UNLIKELY(!pNotebook)) {
                    errorDescription.setBase(QStringLiteral("No notebook was "
                                                            "found for note"));
                    res = qevercloud::EDAMErrorCode::DATA_CONFLICT;
                }
                else if (pNotebook->hasLinkedNotebookGuid())
                {
                    res = checkLinkedNotebookAuthTokenForNotebook(note.notebookGuid(),
                                                                  request.m_authToken,
                                                                  errorDescription);
                }
                else if (!request.m_authToken.isEmpty() &&
                         (request.m_authToken != m_data->m_authenticationToken))
                {
                    errorDescription.setBase(QStringLiteral("Note's notebook doesn't "
                                                            "belong to a linked "
                                                            "notebook but linked "
                                                            "notebook auth token "
                                                            "is not empty"));
            ...
            Q_EMIT getNoteAsyncFinished(res, note.qevercloudNote(),
                                        rateLimitSeconds, errorDescription);
        }
        else
        {
    ...
    {
        QNDEBUG(QStringLiteral("getResourceAsync delay timer event, timer id = ")
                << pEvent->timerId());
        ...
        {
            GetResourceAsyncRequest request =
                m_data->m_getResourceAsyncRequests.dequeue();

            qint32 rateLimitSeconds = 0;
            ErrorString errorDescription;
            Resource resource;
            resource.setGuid(request.m_resourceGuid);
            qint32 res = getResource(request.m_withDataBody,
                                     request.m_withRecognitionDataBody,
                                     request.m_withAlternateDataBody,
                                     request.m_withAttributes,
                                     request.m_authToken, resource,
                                     errorDescription, rateLimitSeconds);
            Q_EMIT getResourceAsyncFinished(res, resource.qevercloudResource(),
                                            rateLimitSeconds, errorDescription);
        }
        else
        {
...
qint32 FakeNoteStore::checkNotebookFields(const Notebook & notebook,
                                          ErrorString & errorDescription) const
{
    ...
    if (notebookName != notebookName.trimmed()) {
        errorDescription.setBase(QStringLiteral("Notebook name cannot begin or "
                                                "end with whitespace"));
        return qevercloud::EDAMErrorCode::BAD_DATA_FORMAT;
    }

    // NOTE: tried to use qevercloud::EDAM_NOTEBOOK_NAME_REGEX to check the name
    // correctness but it appears that regex is too perlish for QRegExp to handle
    // i.e. QRegExp doesn't recognize the regexp in its full entirety and hence
    ...
    if (notebook.hasStack())
    {
        ...
        if (notebookStack.size() < qevercloud::EDAM_NOTEBOOK_STACK_LEN_MIN) {
            errorDescription.setBase(QStringLiteral("Notebook stack's length "
                                                    "is too small"));
            return qevercloud::EDAMErrorCode::BAD_DATA_FORMAT;
        }
        ...
        if (notebookStack != notebookStack.trimmed()) {
            errorDescription.setBase(QStringLiteral("Notebook stack should not "
    ...
    if (notebook.hasPublishingUri())
    {
        ...
        if (publishingUri.size() < qevercloud::EDAM_PUBLISHING_URI_LEN_MIN) {
            errorDescription.setBase(QStringLiteral("Notebook publishing uri "
                                                    "length is too small"));
            return qevercloud::EDAMErrorCode::BAD_DATA_FORMAT;
        }

        if (publishingUri.size() > qevercloud::EDAM_PUBLISHING_URI_LEN_MAX) {
            errorDescription.setBase(QStringLiteral("Notebook publising uri "
                                                    "length is too large"));
            return qevercloud::EDAMErrorCode::BAD_DATA_FORMAT;
        }
        ...
        {
            if (publishingUri == *it) {
                errorDescription.setBase(QStringLiteral("Prohibited publishing "
                                                        "URI value is set"));
                return qevercloud::EDAMErrorCode::BAD_DATA_FORMAT;
            }
        }
        ...
        if (!publishingUriRegExp.exactMatch(publishingUri)) {
            errorDescription.setBase(QStringLiteral("Publishing URI doesn't match "
                                                    "the mandatory regex"));
            return qevercloud::EDAMErrorCode::BAD_DATA_FORMAT;
        }
    }
    ...
    if (notebook.hasPublishingPublicDescription())
    {
        ...
        if (description.size() < qevercloud::EDAM_PUBLISHING_DESCRIPTION_LEN_MIN) {
            errorDescription.setBase(QStringLiteral("Publishing description length "
                                                    "is too small"));
            return qevercloud::EDAMErrorCode::BAD_DATA_FORMAT;
        }
        ...
        if (!publishingDescriptionRegExp.exactMatch(description)) {
            errorDescription.setBase(QStringLiteral("Notebook description doesn't "
...
qint32 FakeNoteStore::checkNoteFields(const Note & note,
                                      ...
                                      ErrorString & errorDescription) const
{
    ...
    if (purpose == CheckNoteFieldsPurpose::CreateNote)
    {
        if (!notebook.canCreateNotes()) {
            errorDescription.setBase(QStringLiteral("No permission to create "
    ...
    else if (purpose == CheckNoteFieldsPurpose::UpdateNote)
    {
        if (!notebook.canUpdateNotes()) {
            errorDescription.setBase(QStringLiteral("No permission to update notes "
    ...
    if (note.hasTitle())
    {
        ...
        // correctness but it appears that regex is too perlish for QRegExp
...
#define CHECK_STRING_ATTRIBUTE(attr) \
        ...
        {
            qint32 res = checkAppData(attributes.applicationData.ref(),
                                      errorDescription);
            if (res != 0) {
                return res;
            }
...
qint32 FakeNoteStore::checkResourceFields(const Resource & resource,
                                          ErrorString & errorDescription) const
{
    if (resource.hasMime())
    {
...
qint32 FakeNoteStore::checkAppData(const qevercloud::LazyMap & appData,
                                   ErrorString & errorDescription) const
{
...
qint32 FakeNoteStore::checkAppDataKey(const QString & key,
                                      const QRegExp & keyRegExp,
                                      ErrorString & errorDescription) const
{
                                       ...
                                       ErrorString & errorDescription)
{
    ...
    {
        savedSearchIt = std::upper_bound(savedSearchUsnIndex.begin(),
                                         savedSearchUsnIndex.end(),
                                         afterUSN, CompareByUSN<SavedSearch>());
    }

    auto tagIt = tagUsnIndex.end();
    if (filter.includeTags.isSet() && filter.includeTags.ref())
    {
        tagIt = std::upper_bound(tagUsnIndex.begin(), tagUsnIndex.end(),
                                 afterUSN, CompareByUSN<Tag>());
    ...
    {
        notebookIt = std::upper_bound(notebookUsnIndex.begin(),
                                      notebookUsnIndex.end(),
                                      afterUSN, CompareByUSN<Notebook>());
        notebookIt = advanceIterator(notebookIt, notebookUsnIndex,
                                     linkedNotebookGuid);
    }
    ...
    {
        noteIt = std::upper_bound(noteUsnIndex.begin(), noteUsnIndex.end(),
                                  afterUSN, CompareByUSN<Note>());
    ...
    {
        resourceIt = std::upper_bound(resourceUsnIndex.begin(),
                                      resourceUsnIndex.end(),
                                      afterUSN, CompareByUSN<Resource>());
    ...
    {
        linkedNotebookIt = std::upper_bound(linkedNotebookUsnIndex.begin(),
                                            linkedNotebookUsnIndex.end(),
    ...
    const QString & linkedNotebookGuid)
{
    ...
    if (linkedNotebookGuid.isEmpty())
    {
        const SavedSearchDataByGuid & savedSearchGuidIndex =
            m_data->m_savedSearches.get<SavedSearchByGuid>();
    ...
    const NotebookDataByGuid & notebookGuidIndex =
        m_data->m_notebooks.get<NotebookByGuid>();
    for(auto it = notebookGuidIndex.begin(),
        end = notebookGuidIndex.end(); it != end; ++it)
    {
        const Notebook & notebook = *it;
        if (linkedNotebookGuid.isEmpty() == notebook.hasLinkedNotebookGuid()) {
            continue;
    ...
    const ResourceDataByGuid & resourceGuidIndex =
        m_data->m_resources.get<ResourceByGuid>();
    for(auto it = resourceGuidIndex.begin(),
        end = resourceGuidIndex.end(); it != end; ++it)
    {
        const Resource & resource = *it;
        auto noteIt = noteGuidIndex.find(resource.noteGuid());
        if (Q_UNLIKELY(noteIt == noteGuidIndex.end())) {
