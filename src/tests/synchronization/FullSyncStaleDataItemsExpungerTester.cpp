...
{
    CompareItemByGuid(QString targetGuid) :
...
void FullSyncStaleDataItemsExpungerTester::init()
{
    ...
    m_pNotebookSyncCache = new NotebookSyncCache(
        *m_pLocalStorageManagerAsync,
        QString(),
        this);

    m_pTagSyncCache = new TagSyncCache(
        *m_pLocalStorageManagerAsync,
        QString(),
        this);

    m_pSavedSearchSyncCache = new SavedSearchSyncCache(
        *m_pLocalStorageManagerAsync,
        this);
}
...
void FullSyncStaleDataItemsExpungerTester::setupBaseDataItems()
{
    ...
    bool res = pLocalStorageManager->addNotebook(
        firstNotebook,
        errorDescription);
    ...
    const QList<Note> & nonSyncedNotes)
{
    ...
    {
        ErrorString errorDescription;
        bool res = pLocalStorageManager->addSavedSearch(
            search,
            errorDescription);
        if (!res) {
            QFAIL(qPrintable(errorDescription.nonLocalizedString()));
        ...
        EventLoopWithExitStatus::ExitStatus::Failure;
    {
        ...
        slotInvokingTimer.singleShot(
            0,
            &expunger,
            &FullSyncStaleDataItemsExpunger::start);
    ...
    else if (expungerTestStatus ==
             EventLoopWithExitStatus::ExitStatus::Timeout)
    {
    ...
    for(const auto & notebook: qAsConst(remainingNotebooks))
    {
        if (notebook.hasGuid())
        {
            auto guidIt = m_syncedGuids.m_syncedNotebookGuids.find(
                notebook.guid());
            ...
            QFAIL("Found a non-synced notebook which survived the purge "
    ...
    for(const auto & tag: qAsConst(remainingTags))
    {
        if (tag.hasGuid())
        {
            ...
            QFAIL("Found a non-synced tag which survived the purge performed "
                  "by FullSyncStaleDataItemsExpunger and kept its guid");
        }

        if (!tag.isDirty()) {
            QFAIL("Found a non-synced and non-dirty tag which survived "
                  "the purge performed by FullSyncStaleDataItemsExpunger");
        }

        auto extraTagIt = std::find_if(
            nonSyncedTags.constBegin(),
    ...
    for(const auto & tag: qAsConst(nonSyncedTags))
    {
        ...
        if ((remainingTagIt == remainingTags.constEnd()) &&
            tag.isDirty())
        {
            QFAIL("One of extra tags which was dirty has not survived "
                  "the purge performed by FullSyncStaleDataItemsExpunger even "
                  "though it was intended to be preserved");
        }
                 ...
                 !tag.isDirty())
        {
            QFAIL("One of extra tags which was not dirty has survived "
                  "the purge performed by FullSyncStaleDataItemsExpunger even "
                  "though it was intended to be expunged");
        }
    }
    ...
    for(const auto & search: qAsConst(remainingSavedSearches))
    {
        if (search.hasGuid())
        {
            auto guidIt = m_syncedGuids.m_syncedSavedSearchGuids.find(
                search.guid());
        ...
        if (!search.isDirty()) {
            QFAIL("Found a non-synced and non-dirty saved search which "
    ...
    for(const auto & search: qAsConst(nonSyncedSavedSearches))
    {
        ...
        if ((remainingSavedSearchIt == remainingSavedSearches.constEnd()) &&
            search.isDirty())
        {
            QFAIL("One of extra saved searches which was dirty has not "
                  "survived the purge performed by "
                  "FullSyncStaleDataItemsExpunger even though it was intended "
                  "to be preserved");
        }
        else if ((remainingSavedSearchIt !=
                  remainingSavedSearches.constEnd()) && !search.isDirty())
        {
            QFAIL("One of extra saved searches which was not dirty has "
                  "survived the purge performed by "
                  "FullSyncStaleDataItemsExpunger even though it was intended "
                  "to be expunged");
        }
        ...
        qAsConst(m_syncedGuids.m_syncedSavedSearchGuids))
    {
        ...
        if (remainingSavedSearchIt == remainingSavedSearches.constEnd()) {
            QFAIL("Could not find a saved search within the remaining ones "
    ...
    for(const auto & note: qAsConst(remainingNotes))
    {
        ...
        if (note.hasGuid())
        {
            ...
            QFAIL("Found a non-synced note which survived the purge performed "
                  "by FullSyncStaleDataItemsExpunger and kept its guid");
        }

        if (!note.isDirty()) {
            QFAIL("Found a non-synced and non-dirty note which survived "
                  "the purge performed by FullSyncStaleDataItemsExpunger");
        }
    }
    ...
    {
        if (!note.hasNotebookGuid() && !note.hasNotebookLocalUid()) {
            QFAIL("One of non-synced notes has no notebook guid and no "
                  "notebook local uid");
        }
        ...
        {
            QFAIL("One of extra notes which was dirty has not survived "
                  "the purge performed by FullSyncStaleDataItemsExpunger even "
                  "though it was intended to be preserved");
        }
        else if ((remainingNoteIt != remainingNotes.constEnd()) &&
                 !note.isDirty())
        {
            QFAIL("One of extra notes which was not dirty has survived "
                  "the purge performed by FullSyncStaleDataItemsExpunger even "
                  "though it was intended to be expunged");
        }
    }
