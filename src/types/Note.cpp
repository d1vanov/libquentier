...
bool Note::validateTitle(const QString & title, ErrorString * pErrorDescription)
{
    if (title != title.trimmed())
    {
        if (pErrorDescription) {
            pErrorDescription->setBase(QT_TRANSLATE_NOOP("", "Note title cannot start or end with whitespace"));
    ...
    {
        if (pErrorDescription) {
            pErrorDescription->setBase(QT_TRANSLATE_NOOP("", "Note title's length is too small"));
            pErrorDescription->details() = title;
        }
    ...
    {
        if (pErrorDescription) {
            pErrorDescription->setBase(QT_TRANSLATE_NOOP("", "Note title's length is too large"));
            pErrorDescription->details() = title;
        }
...
bool Note::checkParameters(ErrorString & errorDescription) const
{
    if (localUid().isEmpty() && !d->m_qecNote.guid.isSet()) {
        errorDescription.setBase(QT_TRANSLATE_NOOP("", "Both Note's local and remote guids are empty"));
        return false;
    }
