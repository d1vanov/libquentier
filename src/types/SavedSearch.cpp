...
bool SavedSearch::validateName(const QString & name, ErrorString * pErrorDescription)
{
    if (name != name.trimmed())
    {
        if (pErrorDescription) {
            pErrorDescription->base() = QString::fromUtf8(QT_TRANSLATE_NOOP("", "Saved search name cannot start or end with whitespace"));
    ...
    {
        if (pErrorDescription) {
            pErrorDescription->base() = QString::fromUtf8(QT_TRANSLATE_NOOP("", "Saved search name's length is too small"));
            pErrorDescription->details() = name;
        }
    ...
    {
        if (pErrorDescription) {
            pErrorDescription->base() = QString::fromUtf8(QT_TRANSLATE_NOOP("", "Saved search's name length is too large"));
            pErrorDescription->details() = name;
        }
...
bool SavedSearch::checkParameters(ErrorString & errorDescription) const
{
    if (localUid().isEmpty() && !d->m_qecSearch.guid.isSet()) {
        errorDescription.base() = QString::fromUtf8(QT_TRANSLATE_NOOP("", "Both saved search's local and remote guids are empty"));
        return false;
    }
