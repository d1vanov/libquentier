...
bool SavedSearchData::checkParameters(ErrorString & errorDescription) const
{
    if (m_qecSearch.guid.isSet() && !checkGuid(m_qecSearch.guid.ref())) {
        errorDescription.base() = QString::fromUtf8(QT_TRANSLATE_NOOP("", "Saved search's guid is invalid"));
    ...
    if (m_qecSearch.updateSequenceNum.isSet() && !checkUpdateSequenceNumber(m_qecSearch.updateSequenceNum)) {
        errorDescription.base() = QString::fromUtf8(QT_TRANSLATE_NOOP("", "Saved search's update sequence number is invalid"));
    ...
    if (m_qecSearch.query.isSet())
    {
        ...
        {
            errorDescription.base() = QString::fromUtf8(QT_TRANSLATE_NOOP("", "Saved search's query exceeds allowed size"));
    ...
    if (m_qecSearch.format.isSet() && (static_cast<qevercloud::QueryFormat::type>(m_qecSearch.format) != qevercloud::QueryFormat::USER)) {
        errorDescription.base() = QString::fromUtf8(QT_TRANSLATE_NOOP("", "Saved search has unsupported query format"));
